...
.initial-screen .btn.p {
  padding: 12px 20px;
  font-size: 16px;
}
.initial-screen .btn.p:hover {
  background: var(--accent-light);
  box-shadow: 0 2px 6px rgba(171,134,91,0.3);
}
.f {
  font-size: 11px;
  padding: 8px;
...
<div class="c" id="cb" role="dialog" aria-label="Chatbot da Frame" aria-live="polite">
  ...
  <div class="b" id="bd">
    ...
    <div id="scrollBottomBtn" class="scroll-bottom-btn" style="display:none;" onclick="scrollToBottom()">Ver mais recente <svg viewBox="0 0 24 24" width="16" height="16"><path d="M12 5v14M5 12l7 7 7-7" fill="none" stroke="currentColor" stroke-width="2"/></svg></div>
  </div>
...
<div class="o" id="ageConfirmOverlay" role="dialog" aria-modal="true">
  <div class="oc">
    ...
    <input type="date" id="birthDate" aria-label="Data de nascimento">
...
function initChat() {
  b.innerHTML = '<div class="loading-spinner" style="display: block;"></div>';
  ...
  if (localStorage.getItem('chatHistory')) {
    try {
      messageHistory = JSON.parse(LZString.decompress(localStorage.getItem('chatHistory'))) || [];
    } catch (e) {
      messageHistory = [];
    }
  }
  ...
  userInput.addEventListener('input', resetInactivity);
  resetInactivity();
}
...
function showInitialScreen() {
  b.innerHTML = `
    <div class="initial-screen">
      <p>Mensagens</p>
      <div class="privacy-note">No atendimento, podemos solicitar dados adicionais conforme necessário. Acesse nossa <a href="https://frameag.com/privacy" target="_blank" class="link">Política de Privacidade</a> no site.</div>
      <div class="recent-msg" id="recentMsg" style="display:none;"></div>
      <button onclick="startChat()" class="btn p">Iniciar Atendimento</button>
      <div class="recent-msg faq-msg" style="background: var(--accent-light); color: #fff; cursor: pointer;" onclick="openFaqPopup()">
        <img class="faq-logo" src="https://framerusercontent.com/images/nqe8sytY941OUcgvF17Y9qLajUc.png" alt="FAQ Logo">
        <span class="faq-text">Tire suas dúvidas mais frequentes <svg class="faq-arrow" viewBox="0 0 24 24" width="12" height="12"><path d="M9 5l7 7-7 7" fill="none" stroke="currentColor" stroke-width="2"/></svg></span>
      </div>
    </div>
  `;
...
function am(text, btn = null, delay = 0, user = false, timestamp = new Date().toISOString(), error = false, errorText = '') {
  let typingEl;
  ...
  setTimeout(() => {
    ...
    m.className = 'msg' + (user ? ' user-msg' : '') + (error ? ' error-msg' : '');
    ...
    if (error && errorText) {
      ...
      alertDiv.innerHTML = '<svg viewBox="0 0 24 24" width="12" height="12"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" fill="var(--error-text)"/></svg> ' + errorText;
      bb.appendChild(alertDiv);
    }
    if (user) {
      m.append(bb);
    } else {
...
function processUserMessage(text) {
  ...
  if (text.length > MAX_MESSAGE_LENGTH) {
    am(text, null, 0, true, new Date().toISOString(), true, 'Limite de caracteres atingido');
    return;
  }
  const prohibited = /(cu|pau|ppk|goza|chupa|puta|kids|kid|baby|bebe|cp|porno|estupro|estuprador|buceta|caralho|foder|fode|transar|sexo|bucetinha|roubar|exterminar|cuzinho|chupetinha|viado|baitola|prostituta|pedofilo|bct|nuds|leitada|fetiche|penis|vagina|se fuder|pepeka|piroca|gozada|pedofilia|violencia|ameaca|drogas|ilegal)/gi;
  if (prohibited.test(text.toLowerCase())) {
    text = text.replace(prohibited, match => match[0] + '*'.repeat(match.length - 1));
    am(text, null, 0, true, new Date().toISOString(), true);
    badWordCount++;
    if (badWordCount === 1) {
  ...
  let masked = false;
  text = text.replace(phoneRegex, (match) => { masked = true; return '**' + match.slice(-4); });
  text = text.replace(cpfRegex, (match) => { masked = true; return '**' + match.slice(-2); });
  if (masked) {
    am(text, null, 0, true);
    am('Por privacidade, mascarei seus dados sensíveis. Para enviar detalhes como telefone ou CPF, use o email contato@frameag.com para maior segurança.');
    return;
  }
  loadingSpinner.style.display = 'block';
  setTimeout(() => {
  ...
  const recent = messageHistory.slice(-5).map(m => m.text.toLowerCase()).join(' ');
  let prefix = '';
  if (recent.includes('premium')) prefix = 'Continuando sobre o Premium, ';
  else if (recent.includes('cadastro')) prefix = 'Continuando sobre o cadastro, ';
  else if (recent.includes('seguranca')) prefix = 'Continuando sobre segurança, ';
  const keywords = {
  ...
  if (keywords.greeting.test(t)) {
    am(prefix + `Olá! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.bomdia.test(t)) {
    am(prefix + `Bom dia! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boatarde.test(t)) {
    am(prefix + `Boa tarde! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boanoite.test(t)) {
    am(prefix + `Boa noite! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.denunciar.test(t)) {
    am(prefix + `Na Frame, levamos segurança muito a sério. Caso encontre irregularidades em anúncios do nosso site, acesse <a href="https://frameag.com/report" target="_blank" class="link">frameag.com/report</a> e denuncie anonimamente. Isso ajuda a manter a plataforma segura para todos.${isPositive ? ' Obrigada pela colaboração!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.humano.test(t)) {
    fh();
    am(prefix + `Se preferir, você também pode enviar um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a>. Nossa equipe está pronta para ajudar com qualquer dúvida mais complexa.${isPositive ? ' 😊' : ''}`);
    return;
  }
  if (keywords.seguranca.test(t)) {
    am(prefix + `Você pode ter se perguntado se a Frame é segura, e sim, é. Aqui, garantimos autenticação em três etapas e contamos com um time de segurança disponível 24h. Além disso, usamos tecnologias avançadas para proteger seus dados e interações. Saiba mais em <a href="https://frameag.com/verificacao" target="_blank" class="link">Blog de Verificação</a>.${isPositive ? ' Fico feliz em esclarecer!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.termos.test(t)) {
    am(prefix + `Nossos Termos e Condições detalham como usamos e protegemos seus dados. Você pode acessá-los em <a href="https://frameag.com/termos" target="_blank" class="link">frameag.com/termos</a>. Para privacidade, focamos em conformidade com leis como LGPD, garantindo transparência e segurança.${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.contratante.test(t)) {
    ...
    am(prefix + `Como contratante, você tem acesso a perfis completos, agendamentos e comunicação segura. O Premium desbloqueia catálogo sem anúncios, galerias exclusivas e suporte prioritário. Visite <a href="https://frameag.com/premium" target="_blank" class="link">frameag.com/premium</a> para mais detalhes e assinatura.${isPositive ? ' Incrível, né?' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.dados.test(t)) {
    am(prefix + `Tratamos dados com total segurança. Você pode acessar, corrigir ou excluir dados via privacidade@frameag.com. Para casos de banimento, retemos dados necessários para prevenção de fraudes, mas sempre com transparência.${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.definicoes.test(t)) {
    am(prefix + `A Frame é uma plataforma especializada em conectar criadoras a contratantes, oferecendo ferramentas de gestão, promoção de anúncios e segurança de ponta. Atuamos como provedora de tecnologia, sem intermediar negociações diretas.${isPositive ? ' Legal, né?' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.pagamento.test(t)) {
    am(prefix + `Para dúvidas sobre pagamentos, acesse sua área de login ou entre em contato com o time via <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> .${isPositive ? ' Vamos resolver rapidinho!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.cancelamento.test(t)) {
    am(prefix + `Para cancelamentos de renovação automática, revise os termos e solicite enviando um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> ${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.ajuda.test(t)) {
    am(prefix + 'Aqui vai uma lista de tópicos que posso ajudar: segurança, cadastro, Frame premium, suporte humano, criadora, contratante, denúncias, Termos, dados pessoais. Digite um deles para mais infos!');
    return;
  }
  if (keywords.finalizar.test(t)) {
    am(prefix + 'Gostaria de encerrar esse atendimento? Sem problemas! Ficamos a disposição para te auxiliar sempre que precisar.');
    return;
  }
  ...
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
    am(prefix + `Não consegui entender sua solicitação. Vamos tentar novamente? Me descreva mais detalhes sobre o que precisa, como segurança e cadastro.${isPositive ? ' 😊' : ''}`, [
...
function cmPT() {
  ...
  am("Selecione uma opção abaixo para prosseguirmos:", [
    ...
    { l: "Contestar decisões", a: () => { am("Possui alguma decisão da moderação ou segurança que deseja contestar? Acesse nosso formulário oficial para análise prioritária pelo Time de Segurança.", 
        [{ l: "Ir para contestar", p: 1, a: () => showRedirectLoading('https://frameag.com/contestar', true) }, 
         { l: "Voltar", a: cmPT }], 
        0); } }
  ], 800);
}
function cadastroPT() {
  ...
  am("Qual dessas situações melhor se aplica ao seu caso? Escolha para eu guiar você.", [
    ...
    { l: "Documentos obrigatórios", a: docsObrigatorios },
    { l: "Voltar", a: cmPT }
  ], 800);
}
function erroCodigo() {
  am("**Erro de Código Inválido**<br><br>1) Verifique email/spam.<br><br>2) Tente reenviar em frameag.com/cadastro.<br><br>3) Use outro email/dispositivo.", null, 0);
  am("Ainda? Vídeo tutorial: <a href='https://frameag.com/ajuda/cadastro' class='link'>Assista aqui</a>", [{ l: "Voltar", a: cadastroPT }], 800);
  perguntarSatisfacao();
}
function falhaAuthentic() {
  am("**Falha no Frame Authentic (Cláusula 4.1.c)**<br><br>• Boa iluminação/rosto nítido.<br><br>• Sem óculos/chapéu.<br><br>• Conexão estável.<br><br>• Tente novamente após 5min.", null, 0);
  am("Suporte: <a href='https://t.me/suporteframebot' class='link'>Telegram</a>", [{ l: "Voltar", a: cadastroPT }], 800);
  perguntarSatisfacao();
}
function docsObrigatorios() {
  am("**Documentos Obrigatórios (Cláusula 4.1.b)**<br><br>• RG/CNH (frente + verso).<br><br>• Selfie segurando o doc (rosto visível).<br><br>• + Verificação facial.", null, 0);
...
function problemaCodigo() {
  ...
  am("1. <span class='semibold'>Verifique a caixa de spam</span> ou lixo eletrônico no seu e-mail. <br><br>2. <span class='semibold'>Confirme se o e-mail cadastrado está correto</span> no site. <br><br>3. <span class='semibold'>Aguarde alguns minutos</span>, pois pode haver atraso. <br><br>4. <span class='semibold'>Tente reenviar o código</span> diretamente pela página de cadastro.", null, 800);
...
function problemaEmail() {
  ...
  am("1. <span class='semibold'>Tente recuperar a conta de e-mail</span> diretamente no provedor (como Gmail ou Outlook). <br><br>2. <span class='semibold'>Se não for possível</span>, podemos ajudar a atualizar o e-mail na sua conta Frame.", null, 800);
  am("Importante: Para segurança, você precisará confirmar dados pessoais e passar pelo Frame Authentic antes da atualização.", null, 1600);
...
function premiumPT() {
  am("**Torne-se Premium e desbloqueie tudo!**<br><br>• Catálogo completo sem anúncios.<br><br>• FramePay com lounges VIP e hotéis parceiros.<br><br>• Atendimento prioritário 24h.", null, 0);
  am("Clique e conheça planos:", [{ l: "Quero ser Premium", p: 1, a: () => showRedirectLoading('https://frameag.com/premium', true) }, { l: "Voltar", a: menuContratantePosIdade }], 800);
  perguntarSatisfacao();
}
function ctPT() {
...
function agendarExperiencia() {
  am("**Agendar Experiência Personalizada (Cláusula 5.2.1)**<br><br>• **Virtual**: Videochamadas, chats exclusivos via app.<br><br>• **Presencial**: Negociado diretamente (consenso total).<br><br>• **Passos**: 1) Catálogo → 2) Chat ao vivo → 3) Confirma agenda + pagamento.", null, 0);
  am("A Frame só fornece a tecnologia - negociação é 100% direta e segura entre você e a criadora.", 
...
function visitarCatalogo() {
  ...
  am("Decidiu? Fale com nosso time:", [{ l: "Ir para atendimento", p: 1, a: () => showRedirectLoading('https://t.me/suporteframebot?start=catalogo-site', true) }], 800);
  perguntarSatisfacao();
}
function verificacaoContratante() {
  am("**Frame Authentic: Sua Proteção Garantida (Cláusula 4)**<br><br>• Verificação facial ao vivo.<br><br>• Documentos + selfie obrigatórios.<br><br>• IA + Time de Segurança revisa tudo mensalmente.<br><br>• **Resultado**: Zero fraudes, 100% perfis reais.", null, 0);
  am("Contratantes também podem ser verificados em caso de denuncias ou pendencias cadastrais.", [{ l: "Entendi", a: ctPT }], 800);
  perguntarSatisfacao();
}
function crPT() {
  ...
  am("Selecione uma opção abaixo para prosseguirmos:", [
    ...
    { l: "Gerenciar Brand Page/perfil", a: gerenciarPerfil },
    { l: "Como usar Frame Payments", a: framePayments },
    { l: "Contestar banimento de conta", a: contestarBan },
    { l: "Voltar", a: crPT },
    { l: "Contestar decisões", a: () => { am("Possui alguma decisão da moderação ou segurança que deseja contestar? Acesse nosso formulário oficial para análise prioritária pelo Time de Segurança.", 
        [{ l: "Ir para contestar", p: 1, a: () => showRedirectLoading('https://frameag.com/contestar', true) }, 
         { l: "Voltar", a: crPT }], 
...
function gerenciarPerfil() {
  am("**Gerenciar Brand Page (Cláusula 5.2.6)**<br><br>• Edite fotos, bio, medidas, redes.<br><br>• Destaque no catálogo com Premium.<br><br>• Acompanhe visualizações/agendamentos.", null, 0);
  am("Acesse: <a href='https://frameag.com/login' class='link'>Área Login</a>", [{ l: "Voltar", a: crPT }], 800);
  perguntarSatisfacao();
}
function framePayments() {
  am("**Frame Payments: Receba Direto (Cláusula 15.7)**<br><br>• Bot Telegram para quitação instantânea.<br><br>• Sem % da Frame - 100% seu.<br><br>• Seguro: Verificação antes do pagamento.", null, 0);
  am("Ative no app: t.me/suporteframebot", [{ l: "Voltar", a: crPT }], 800);
  perguntarSatisfacao();
}
function contestarBan() {
  am("**Contestar Banimento**<br><br>Envie prints + explicação para análise em 24h pelo Time de Segurança.", null, 0);
...
function handleScroll() {
  ...
  if (b.scrollHeight - b.scrollTop - b.clientHeight > 100) {
...
window.onload = () => { b.innerHTML = '<div class="loading-spinner" style="display: block;"></div>'; };
// ===== Funções adicionais =====
function cleanStorageIfNeeded() {
  let size = 0;
  for (let k in localStorage) {
    if (localStorage.hasOwnProperty(k)) {
      size += (localStorage[k].length + k.length) * 2; // UTF-16 approximation
    }
  }
  try {
...
function checkBlockStatus() {
  if (isChatBlocked) {
    ...
    if (blockType === 'behavior') {
      ...
      showBlockPopup('Chat bloqueado por declaração de idade inapropriada para os serviços da Frame Agency. Envie email para liberação ou confirme sua idade.');
      document.getElementById('ageConfirmBtn').style.display = 'block';
    }
  }
}
...
function openFaqPopup() {
  ...
  const faqs = [
    ...
    { q: "Como me cadastro como criadora?", a: "Para se cadastrar, você precisa ter mais de 18 anos, fornecer documentos como RG ou CNH, uma selfie e passar pela verificação facial. O processo é simples e pode ser iniciado no nosso site." },
    ...
    { q: "O que é Frame Payments?", a: "É uma ferramenta no Telegram para receber pagamentos diretamente, sem taxas da Frame. É seguro e verificado antes de cada transação." },
    { q: "Como denuncio algo suspeito?", a: "Use o formulário em frameag.com/report para reportar anonimamente. Nosso time de segurança analisa rapidamente." },
    { q: "Meus dados estão seguros?", a: "Sim, usamos criptografia avançada e cumprimos leis como LGPD. Dados são usados só para o necessário e você pode solicitar acesso ou exclusão." },
    { q: "Como contesto uma decisão?", a: "Acesse frameag.com/contestar com detalhes e provas. O time de segurança revisa em até 24h." }
  ];
  faqs.forEach(faq => {
