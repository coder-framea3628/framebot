    ...
    b.innerHTML = `
      <div class="mini-form">
        ...
        <label class="checkbox-label">
          ...
          Declaro ser maior de 18 anos e aceito os <a href="https://frameag.com/termos" target="_blank" class="link">Termos e Condições</a>
        </label>
...
function saveUserInfo() {
  ...
  if (ageCheckbox && !ageCheckbox.checked) {
    showWarningPopup('Você deve declarar ser maior de 18 anos e aceitar os termos para continuar.');
    return;
  }
  if (validateUserInput(name, email)) {
    ...
    if (badFormAttempts === 3) {
      showWarningPopup('Após 3 tentativas inválidas, por favor, corrija os campos destacados.');
    }
  }
}
...
function processUserMessage(text) {
  ...
  if (prohibited.test(text.toLowerCase())) {
    ...
    if (badWordCount === 1) {
      ...
      am('Por favor, mantenha a conversa respeitosa. Estou aqui para ajudar com tópicos adequados.');
    } else {
...
function problemaCodigo() {
  ...
  am("Verifique a caixa de spam ou lixo eletrônico no seu e-mail.");
  am("Confirme se o e-mail cadastrado está correto no site.");
  am("Aguarde alguns minutos, pois pode haver atraso.");
  am("Tente reenviar o código diretamente pela página de cadastro.");
...
function problemaEmail() {
  ...
  am("Tente recuperar a conta de e-mail diretamente no provedor (como Gmail ou Outlook).");
  am("Se não for possível, podemos ajudar a atualizar o e-mail na sua conta Frame.");
...
function ctPT() {
  am("Antes de prosseguirmos, confirme se você possui 18 anos ou mais, pois nossos serviços são exclusivos para maiores de idade.", [
    { l: "Sou maior de 18 anos", p: 1, a: () => { 
        ...
        am("Se já decidiu qual modelo contratar, fale diretamente com nosso time para assistência rápida e segura:", [{ l: "Ir para atendimento 24h", p: 1, a: () => { showRedirectLoading(); window.open('https://t.me/suporteframebot?start=catalogo-site', '_blank'); } }], 400);
...
function blockChatForUnderage() {
  ...
  localStorage.removeItem('chatHistory');
  localStorage.removeItem('lastMessage');
  showBlockPopup('Chat bloqueado por declaração de menoridade. Envie email para liberação.');
}
...
function blockChatForBehavior() {
  ...
  localStorage.removeItem('chatHistory');
  localStorage.removeItem('lastMessage');
  showBlockPopup('Por comportamento inadequado você poderá utilizar esse chat em: ');
  startBlockTimer();
}
...
function validateUserInput(name, email) {
  ...
  if (prohibited.test(name) || prohibited.test(email)) {
    ...
    lastBadFormMessage.textContent = 'Por favor, evite palavras inadequadas em nomes ou e-mails.';
