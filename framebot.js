...
function agendarExperiencia() {
  typingShownThisFlow = false;
  am('Para agendar, basta selecionar uma criadora no catálogo da Frame Agency, iniciar um chat, negociar os detalhes e assinar o contrato enviado por e-mail. A Frame fornece a tecnologia, a negociação é 100% direta e segura entre vocês.', null, 0);
...
function verificacaoContratante() {
  typingShownThisFlow = false;
  am('O Frame Authentic verifica a identidade das acompanhantes e modelos com envio de documentos, selfie e biometria facial ao vivo. O resultado? Perfis 100% reais e uma plataforma onde fraude é coisa do passado.', null, 0);
...
function crPT() {
  ...
  am('Como você é uma criadora, posso ajudar com os assuntos abaixo:', [
    ...
    { l: 'Como funciona a verificação', a: () => {
      typingShownThisFlow = false;
      am('Você precisa ter mais de 18 anos, passar pelo Frame Authentic (verificação de identidade) e aceitar os nossos Termos. Isso garante segurança para todos os usuários da plataforma.');
      setTimeout(perguntarSatisfacao, 600);
    }},
...
function gerenciarPerfil() {
  typingShownThisFlow = false;
  am('Na sua BrandPage você pode editar suas mídias, detalhes do atendimento, bio, medidas e redes sociais. Para destaque no catálogo, veja os planos de impulsionamento disponíveis em sua dashboard.');
...
function framePayments() {
  typingShownThisFlow = false;
  am('Com o Frame Payments você recebe através do Telegram ou FrameChat e realiza saques diretamente para sua conta, sem taxas da Frame Agency. Com anti-fraude e maior nível de segurança em cada transação.');
...
function clearChatHistory() {
  closeMenu();
  showConfirm('Tem certeza que deseja limpar seu histórico? Ao continuar, ele não poderá ser recuperado.', () => {
    messageHistory = [];
...
function editProfile() {
  closeMenu();
  if (!userInfo) {
    showWarningPopup('Poxa, não localizei nenhum perfil. Inicie um atendimento para criar seu perfil por aqui.');
    return;
  }
...
function setupFaqSheet() {
  const faqs = [
    { q: 'O que é a Frame Agency?', a: 'A Frame Agency é uma plataforma completa que em conecta criadoras a contratantes, oferecendo ferramentas de gestão de perfis, anúncios e interações seguras. Atuamos como provedora de tecnologia com recursos inovadores.' },
    ...
    { q: 'O que é a Verificação Facial?', a: 'É um processo para confirmar que você é real e maior de idade. Usamos a nossa tecnologia própria 'Frame Authentic' para checar documentos e movimentos faciais em tempo real.' },
    { q: 'Como funciona o Frame Premium?', a: 'Com o Premium você acessa o catálogo completo sem anúncios, recebe informações completas de modelos, realiza agendamentos, tem suporte prioritário e benefícios como lounges VIP e hotéis parceiros.' },
    ...
    { q: 'O que é o Frame Payments?', a: 'É um recurso da Frame Agency que automatiza pagamentos no Telegram para que você receber diretamente e de forma automatizada, livre de taxas da Frame. Conta com verificação e anti fraude em cada transação.' },
    { q: 'Como denuncio golpes ou algo suspeito?', a: 'Use o formulário disponível em: frameag.com/report para reportar anonimamente. Nosso time de segurança 24h analisa rapidamente e toma as medidas necessárias.' },
    { q: 'Meus dados estão seguros?', a: 'Sim. Usamos criptografia avançada, cumprindo a GDPR e LGPD. Os seus dados são usados apenas para o necessário (como manter sua conta ativa) e você pode solicitar acesso ou exclusão quando desejar.' },
    { q: 'Como contesto uma decisão da moderação?', a: 'Acesse a página: frameag.com/contestar e preencha o formulário com detalhes. O time de segurança revisará sua apelação em até 7 dias úteis.' }
  ];
