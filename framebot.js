...
let typingShownThisFlow = false; // Variável global para controlar o indicador de digitação
...
function am(text, btn, delay = 0, user = false) {
  if (!user && !typingShownThisFlow) {
    ...
    typingShownThisFlow = true; // Marca que o indicador foi mostrado
  }
  setTimeout(() => {
...
function processUser Message(text) {
  am(text, null, 0, true);
  let t = text.toLowerCase();

  if (t.includes("denunciar")) {
    am('Na Frame, levamos segurança muito a sério. Caso encontre irregularidades, acesse <a href=https://frameag.com/report target=_blank class=link>frameag.com/report</a> e denuncie anonimamente.');
    return;
  }
  ...
  if (t.includes("seguro") || t.includes("segurança")) {
    am('Você pode ter se perguntado se a Frame é segura, e sim, é. Autenticação em três etapas e time 24h <a href=https://frameag.com/blog/verificacao target=_blank class=link>Saiba mais</a>');
...
document.getElementById('userInput').addEventListener('keypress', e => {
  if (e.key === 'Enter' && e.target.value.trim() !== "") {
    processUser Message(e.target.value.trim());
    e.target.value = "";
  }
});
...
document.getElementById('sendBtn').addEventListener('click', () => {
  ...
  if (userInput.value.trim() !== "") {
    processUser Message(userInput.value.trim());
    userInput.value = "";
  }
});
...
function premiumPT() {
  am('No <span class="semibold">Premium</span>, você desbloqueia vantagens únicas.');
  am('Acesso ao catálogo completo, Galerias Sexy, atendimento prioritário e mais.', null, 700);
...
function ctPT() {
  ...
  am("Você possui 18 anos ou mais?", [
    { l: "Sim", p: 1, a: () => { 
        am('Certo. Antes de contratar, veja nosso <a href="https://frameag.com/models" target="_blank" class="link">catálogo completo</a>.');
        am("Se quiser contratar, fale com nosso time:", [{ l: "Ir para atendimento 24h", p: 1, a: () => open('https://t.me/suporteframebot?start=catalogo-site', '_blank') }], 500);
...
function crPT() {
  ...
  am('É necessário ter mais de 18 anos e aceitar nossos <a href=https://frameag.com/termos target=_blank class=link>Termos e Condições</a>.', null, 1400);
  am('Se precisar, acesse sua <a href=https://frameag.com/login target=_blank class=link>Área de Login</a> e clique em "Central de Atendimento".', [
    { l: "Voltar", a: inicio }
  ], 2000);
