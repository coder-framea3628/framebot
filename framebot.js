...
.input-box {
  ...
  position: relative; /* <- ADICIONADO: necessário para posicionar o botão */
  overflow: visible;
}
...
.input-box input {
  ...
  padding-right: 56px; /* <- ADICIONADO: espaço suficiente para o botão (ajusta se o botão mudar de tamanho) */
  min-width: 0; /* evita overflow em flexbox */
}
...
.send-btn {
  ...
  padding: 0;
  transition: all 0.3s ease;
  position: absolute; /* <- alteração importante */
  right: 10px;        /* distância da borda direita da input-box */
  top: 50%;
  transform: translateY(-50%);
  z-index: 3;         /* garante que esteja sobre o background, mas o input já deu espaço */
}
...
function initChat() {
  if (userInfo) {
    ...
    welcomeText.textContent = `Olá! Como podemos ajudar?`;
    showInitialScreen();
  }
...
function showInitialScreen() {
  b.innerHTML = `
    <div class="initial-screen">
      <p>Mensagens</p>
      <div class="privacy-note">No atendimento, podemos solicitar dados e informações adicionais conforme necessário. Acesse nossa Política de Privacidade no site.</div>
...
function processUserMessage(text) {
  ...
  const keywords = {
    ...
    humano: /(humano|analista|agente|suporte|especialista|falar com a gente)/i,
    ...
    definicoes: /(definições|saber mais|oq é|o que é plataforma)/i,
  ...
  if (keywords.contratante.test(t)) {
    ...
    am('Como contratante, você tem acesso a perfis completos, agendamentos e ao FrameChat. O Premium desbloqueia catálogo sem anúncios, galerias exclusivas e suporte prioritário. Visite <a href="https://frameag.com/premium" target="_blank" class="link">frameag.com/premium</a> para mais detalhes e assinatura.');
    perguntarSatisfacao();
    return;
  }
  if (keywords.dados.test(t)) {
    am('Tratamos dados com cuidado, conforme a LGPD. Você pode acessar, corrigir ou excluir dados via privacidade@frameag.com. Para casos de banimento, retemos dados necessários para prevenção de fraudes, mas sempre com transparência.');
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.definicoes.test(t)) {
    am('A Frame é uma plataforma especializada em conectar criadoras de conteúdo com contratantes, oferecendo ferramentas de gestão, classificado de anúncios e segurança. Atuamos como provedora de tecnologia, sem intermediar negociações diretas.');
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.cancelamento.test(t)) {
    am('Para cancelamentos, revise os Termos da Frame e solicite enviando um email para: contato@frameag.com.');
    perguntarSatisfacao();
    return;
  }
...
function perguntarSatisfacao() {
  ...
  am(`Minha explicação ajudou a esclarecer sua dúvida, ${nome}? Estou aqui para explicar mais se necessário.`, [
    { l: "Sim, tudo esclarecido!", p: 1, a: () => {
        am(`Perfeito, ${nome}! Agradecemos pelo contato e estamos sempre à disposição para te ajudar!`);
...
function problemaCodigo() {
  ...
  am("1. Verifique a caixa de spam ou lixo eletrônico no seu e-mail. 2. Confirme se o e-mail cadastrado está correto no site. 3. Aguarde alguns minutos, pois pode haver atraso.<br>4. Tente reenviar o código diretamente pela página de cadastro.");
  am("Após 3 tentativas inválidas, aguarde 24h. Se o problema persistir após essas verificações, entre em contato com nossa equipe de suporte:", [
...
function problemaEmail() {
  ...
  am("1. Tente recuperar a conta de e-mail diretamente no provedor (como Gmail ou Outlook). 2. Se não for possível, podemos ajudar a atualizar o e-mail na sua conta Frame.");
  am("Importante: Para segurança, você precisará confirmar dados pessoais e passar pelo Frame Authentic antes da atualização.");
...
function problemaLogin() {
  am("Para iniciar ou recuperar seu login, acesse a Área de Cadastro em <a href='https://frameag.com/cadastro' target='_blank' class='link'>frameag.com/cadastro</a> e siga os passos.");
  ...
  am("Deseja voltar ao menu 'Problemas com cadastro' para outras opções?", [
...
function ctPT() {
  am("Antes de prosseguirmos, confirme se você possui 18 anos ou mais, pois nossos serviços são exclusivos para maiores de idade.", [
    { l: "Sou maior de 18 anos", p: 1, a: () => { 
        am('Ótimo! Antes de contratar, explore todas as modelos em nosso catálogo completo: <a href="https://frameag.com/models" target="_blank" class="link">frameag.com/models</a>. Lá você encontra perfis verificados e opções variadas.');
        am("Se já decidiu qual modelo contratar, fale diretamente com nosso time para finalizar as etapas de forma rápida e segura:", [{ l: "Ir para atendimento 24h", p: 1, a: () => window.open('https://t.me/suporteframebot?start=catalogo-site', '_blank') }], 400);
...
function clearChatHistory() {
  showConfirm('Tem certeza que deseja limpar o histórico de conversas no chat? Isso não pode ser desfeito.', () => {
    messageHistory = [];
...
function editProfile() {
  showConfirm('Tem certeza que deseja editar seu perfil na Frame? Alterações serão salvas na próxima etapa.', () => {
    b.innerHTML = `
      <div class="mini-form">
...
function exportHistory() {
  ...
  a.href = url;
  a.download = 'transcricao_chat_frameagency.txt';
  a.click();
...
function validateUserInput(name, email) {
  const nameInput = document.getElementById('nameInput');
  const emailInput = document.getElementById('emailInput');
  const prohibited = /(cu|pau|ppk|goza|chupa|puta|kids|kid|baby|bebe|cp|porno|estupro|estuprador|buceta)/i;
  let valid = true;
  if (!name) {
  ...
  if (prohibited.test(name) || prohibited.test(email)) {
    valid = false;
    am('Por favor, evite palavras inadequadas em seu nome ou e-mail. Corrija acima para continuarmos o atendimento.');
  }
  return valid;
}
