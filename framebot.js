...
.bb.error .error-alert svg {
  width: 12px;
  height: 12px;
  fill: var(--error-alert);
}
.bb .obscured {
...
.recent-msg.faq .faq-logo {
  width: 24px;
  height: 24px;
  border-radius: 50%;
}
.recent-msg.faq .faq-text {
  font-size: 13px;
}
.recent-msg.faq .faq-arrow {
  margin-left: 8px;
  width: 16px;
  height: 16px;
}
...
.faq-overlay .oc {
  border: 1px solid var(--earth-tone);
  position: relative;
}
.faq-overlay .oc .frame-logo {
  position: absolute;
  top: 10px;
  left: 10px;
  width: 40px;
...
body.dark .faq-overlay .oc .frame-logo {
  content: url('https://framerusercontent.com/images/Oe8d7JZskbAvjlTrwbIEeozgs.png');
}
.faq-overlay .oc .frame-logo {
  content: url('https://framerusercontent.com/images/pZ3VoR0IVMlUfgFWhXRDczmt7Q.png');
}
`;
...
<div class="c" id="cb" role="dialog" aria-label="Chatbot da Frame" aria-live="polite">
  ...
  <div class="f">
    ...
    com os <a href="https://frameag.com/termos" target="_blank" class="link">Termos e Condições</a> da Frame. Veja nossa <a href="https://frameag.com/privacy" target="_blank" class="link">Política de Privacidade</a>.
  </div>
</div>
...
<div class="scroll-down-btn" id="scrollDownBtn" onclick="scrollToBottom()" aria-label="Ver mais recente">Ver mais recente <svg viewBox="0 0 24 24"><path d="M12 19V5M5 12l7-7 7 7"/></svg></div>
...
<div class="o faq-overlay" id="faqOverlay" role="dialog" aria-modal="true">
  <div class="oc" style="max-width: 80%; max-height: 80%; overflow-y: auto; padding: 20px; text-align: left;">
    ...
    <img class="frame-logo" alt="Logo Frame">
...
let lzReady = false;
lzScript.onload = () => { lzReady = true; loadHistoryIfReady(); };
lzScript.onerror = () => { console.error('Falha ao carregar LZ-String. Usando storage sem compressão.'); lzReady = true; loadHistoryIfReady(); };
...
function loadHistoryIfReady() {
  const compressed = localStorage.getItem('chatHistory');
  messageHistory = compressed ? JSON.parse(LZString.decompress(compressed)) || [] : [];
...
function showInitialScreen() {
  b.innerHTML = `
    <div class="initial-screen">
      ...
      <button onclick="startChat()" class="btn p start-btn">Iniciar Atendimento</button>
      <div class="recent-msg faq" onclick="openFaqPopup()">
        <img class="faq-logo" src="https://framerusercontent.com/images/nqe8sytY941OUcgvF17Y9qLajUc.png" alt="Logo FAQ">
        <span style="font-weight: 600;">Perguntas frequentes</span>
        <span class="faq-text" style="margin-left: auto; font-size: 13px;">Tire suas dúvidas sobre a Frame Agency</span>
        <svg class="faq-arrow" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
      </div>
    </div>
  `;
...
function processUserMessage(text) {
  ...
  let isViolation = prohibited.test(text.toLowerCase());
  let isError = false;
  let errorMsg = '';
  if (text.length > MAX_CHAR_LIMIT) {
    isError = true;
    errorMsg = 'Limite de caracteres atingido';
  }
  if (isError || isViolation) {
    am(text, null, 0, true);
    const lastUserMsg = b.lastChild.querySelector('.bb');
    lastUserMsg.classList.add('error');
    if (isViolation) {
      lastUserMsg.innerHTML = lastUserMsg.innerHTML.replace(prohibited, match => match[0] + '*' + match.slice(2));
    } else {
      const alertEl = document.createElement('span');
      alertEl.className = 'error-alert';
      alertEl.innerHTML = `<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>${errorMsg}`;
      lastUserMsg.appendChild(alertEl);
    }
    if (isViolation) {
      badWordCount++;
      if (badWordCount === 1) {
  ...
  if (phoneRegex.test(text)) {
    am(text, null, 0, true);
    am('Para sua privacidade, sugerimos enviar números de telefone ou dados sensíveis por e-mail para contato@frameag.com em vez de no chat.');
    return;
  }
  ...
  if (keywords.greeting.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Olá! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.bomdia.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Bom dia! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boatarde.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Boa tarde! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boanoite.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Boa noite! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.denunciar.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Na Frame, levamos segurança muito a sério. Caso encontre irregularidades em anúncios do nosso site, acesse <a href="https://frameag.com/report" target="_blank" class="link">frameag.com/report</a> e denuncie anonimamente. Isso ajuda a manter a plataforma segura para todos.${isPositive ? ' Obrigada pela colaboração!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.humano.test(t)) {
    fh();
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Se preferir, você também pode enviar um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a>. Nossa equipe está pronta para ajudar com qualquer dúvida mais complexa.${isPositive ? ' 😊' : ''}`);
    return;
  }
  if (keywords.seguranca.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Você pode ter se perguntado se a Frame é segura, e sim, é. Aqui, garantimos autenticação em três etapas e contamos com um time de segurança disponível 24h. Além disso, usamos tecnologias avançadas para proteger seus dados e interações. Saiba mais em <a href="https://frameag.com/verificacao" target="_blank" class="link">Blog de Verificação</a>.${isPositive ? ' Fico feliz em esclarecer!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.termos.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Nossos Termos e Condições detalham como usamos e protegemos seus dados. Você pode acessá-los em <a href="https://frameag.com/termos" target="_blank" class="link">frameag.com/termos</a>. Para privacidade, focamos em conformidade com leis como LGPD, garantindo transparência e segurança.${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.contratante.test(t)) {
    ...
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Como contratante, você tem acesso a perfis completos, agendamentos e comunicação segura. O Premium desbloqueia catálogo sem anúncios, galerias exclusivas e suporte prioritário. Visite <a href="https://frameag.com/premium" target="_blank" class="link">frameag.com/premium</a> para mais detalhes e assinatura.${isPositive ? ' Incrível, né?' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.dados.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Tratamos dados com total segurança. Você pode acessar, corrigir ou excluir dados via privacidade@frameag.com. Para casos de banimento, retemos dados necessários para prevenção de fraudes, mas sempre com transparência.${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.definicoes.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}A Frame é uma plataforma especializada em conectar criadoras a contratantes, oferecendo ferramentas de gestão, promoção de anúncios e segurança de ponta. Atuamos como provedora de tecnologia, sem intermediar negociações diretas.${isPositive ? ' Legal, né?' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.pagamento.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Para dúvidas sobre pagamentos, acesse sua área de login ou entre em contato com o time via <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> .${isPositive ? ' Vamos resolver rapidinho!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.cancelamento.test(t)) {
    am(`${previousContext ? `Continuando sobre ${previousContext}, ` : ''}Para cancelamentos de renovação automática, revise os termos e solicite enviando um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> ${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
    am(`${previousContext ? `Baseado na sua dúvida anterior sobre ${previousContext}, sugiro... ` : ''}Não consegui entender sua solicitação. Vamos tentar novamente? Me descreva mais detalhes sobre o que precisa, como segurança e cadastro.${isPositive ? ' 😊' : ''}`, [
...
function handleScroll() {
  ...
  scrollDownBtn.classList.toggle('show', b.scrollTop + b.clientHeight < b.scrollHeight - 100);
}
...
function getPreviousContext() {
  ...
  for (let key in contextKeywords) {
    if (lastMessages.some(text => contextKeywords[key].test(text))) {
      return key;
    }
  }
  return null;
