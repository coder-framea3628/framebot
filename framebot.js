...
<div class="o" id="ageConfirmOverlay" role="dialog" aria-modal="true">
  <div class="oc">
    ...
    <input type="number" id="dayInput" placeholder="Dia" min="1" max="31" aria-label="Dia de nascimento">
    <input type="number" id="monthInput" placeholder="Mês" min="1" max="12" aria-label="Mês de nascimento">
    <input type="number" id="yearInput" placeholder="Ano" min="1900" max="${new Date().getFullYear()}" aria-label="Ano de nascimento">
...
let messageHistory = localStorage.getItem('chatHistory') ? JSON.parse(LZString.decompress(localStorage.getItem('chatHistory'))) || [] : [];
...
const emojis = [
  ...
  {emoji: '😆', name: 'rosto sorridente apertando os olhos'},
...
function showInitialScreen() {
  b.innerHTML = `
    <div class="initial-screen">
      ...
      <div class="recent-msg" style="background: var(--accent-light); color: #fff; cursor: pointer;" onclick="openFaqPopup()">
        <span style="font-weight: 600;">Perguntas frequentes</span>
        <span style="margin-left: auto;">Tire suas dúvidas mais frequentes ></span>
      </div>
    </div>
  `;
...
function processUserMessage(text) {
  ...
  if (keywords.greeting.test(t)) {
    am(`Olá! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.bomdia.test(t)) {
    am(`Bom dia! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boatarde.test(t)) {
    am(`Boa tarde! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.boanoite.test(t)) {
    am(`Boa noite! Tudo bem? Como posso ajudar você hoje?${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  if (keywords.denunciar.test(t)) {
    am(`Na Frame, levamos segurança muito a sério. Caso encontre irregularidades em anúncios do nosso site, acesse <a href="https://frameag.com/report" target="_blank" class="link">frameag.com/report</a> e denuncie anonimamente. Isso ajuda a manter a plataforma segura para todos.${isPositive ? ' Obrigada pela colaboração!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.humano.test(t)) {
    fh();
    am(`Se preferir, você também pode enviar um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a>. Nossa equipe está pronta para ajudar com qualquer dúvida mais complexa.${isPositive ? ' 😊' : ''}`);
    return;
  }
  if (keywords.seguranca.test(t)) {
    am(`Você pode ter se perguntado se a Frame é segura, e sim, é. Aqui, garantimos autenticação em três etapas e contamos com um time de segurança disponível 24h. Além disso, usamos tecnologias avançadas para proteger seus dados e interações. Saiba mais em <a href="https://frameag.com/verificacao" target="_blank" class="link">nosso blog sobre verificação</a>.${isPositive ? ' Fico feliz em esclarecer!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.termos.test(t)) {
    am(`Nossos Termos e Condições detalham como usamos e protegemos seus dados. Você pode acessá-los em <a href="https://frameag.com/termos" target="_blank" class="link">frameag.com/termos</a>. Para privacidade, focamos em conformidade com leis como LGPD, garantindo transparência e segurança.${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.definicoes.test(t)) {
    am(`A Frame é uma plataforma especializada em conectar criadoras a contratantes, oferecendo ferramentas de gestão, promoção de anúncios e segurança de ponta. Atuamos como provedora de tecnologia, sem intermediar negociações diretas.${isPositive ? ' Legal, né?' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.pagamento.test(t)) {
    am(`Para dúvidas sobre pagamentos, acesse sua área de login ou entre em contato com o time via suporte.${isPositive ? ' Vamos resolver rapidinho!' : ''}`);
    perguntarSatisfacao();
    return;
  }
  if (keywords.cancelamento.test(t)) {
    am(`Para cancelamentos de renovação automática, revise os termos e solicite enviando um email para: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> ${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    return;
  }
  ...
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
    am(`Não consegui entender sua solicitação. Vamos tentar novamente? Me descreva mais detalhes sobre o que precisa, como segurança e cadastro.${isPositive ? ' 😊' : ''}`, [
...
function erroCodigo() {
  am("**Erro de Código Inválido**<br>1) Verifique email/spam.<br>2) Tente reenviar em frameag.com/cadastro.<br>3) Use outro email/dispositivo.", null, 0);
...
function falhaAuthentic() {
  am("**Falha no Frame Authentic (Cláusula 4.1.c)**<br>• Boa iluminação/rosto nítido.<br>• Sem óculos/chapéu.<br>• Conexão estável.<br>• Tente novamente após 5min.", null, 0);
  am("Suporte: <a href='https://t.me/suporteframebot' class='link'>Telegram</a>", [{ l: "Voltar", a: cadastroPT }], 800);
  perguntarSatisfacao();
}

function docsObrigatorios() {
  am("**Documentos Obrigatórios (Cláusula 4.1.b)**<br>• RG/CNH (frente + verso).<br>• Selfie segurando o doc (rosto visível).<br>• + Verificação facial.", null, 0);
...
function premiumPT() {
  am("**Torne-se Premium e desbloqueie tudo!**<br>• Catálogo completo sem anúncios.<br>• FramePay com lounges VIP e hotéis parceiros.<br>• Atendimento prioritário 24h.", null, 0);
