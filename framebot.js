...
#emojiSearch, #historySearch, #feedbackText, input[type="date"], #searchResults input {
  font-size: 16px !important;
}
#emojiSearch:focus, #historySearch:focus, #feedbackText:focus, input[type="date"]:focus {
...
<div class="c" id="cb" role="dialog" aria-label="Chatbot da Frame" aria-live="polite">
  <div class="h" id="header">
    ...
    <div class="close-chat" role="button" aria-label="Opções de chat">
...
<div class="o" id="ov" role="dialog" aria-modal="true">
  <div class="oc" id="menuOverlay">
    ...
    <input type="text" id="historySearch" placeholder="Buscar no histórico..." oninput="searchHistory()">
...
const intents = {
  ...
  help: {
    ...
    action: () => { am('Aqui vai uma lista de tópicos que posso ajudar: segurança, cadastro, Frame premium, suporte humano, criadora, contratante, denúncias, Termos, dados pessoais. Digite um deles para mais infos!'); }
  }
};
const notUnderstandMsgs = [
  ...
  'Hmm, não entendi bem. Pode explicar melhor sua dúvida?'
];
const greetingMsgs = [
...
const inactivityMsgs = [
  'Ainda aí? Gostaria de saber se ainda posso te ajudar.',
  'Precisa de mais alguma coisa? Estou à disposição.',
  'Tudo certo por aí? Se precisar, é só chamar!'
];
...
function am(text, btn = null, delay = 0, user = false, timestamp = new Date().toISOString()) {
  ...
  setTimeout(() => {
    ...
    // Add copy for links in user messages
    if (user) {
...
function processUserMessage(text) {
  ...
  if (keywords.resolvido.test(t)) {
    am('Fico feliz que pude ajudar! Gostaria de avaliar a experiência?', [
      { l: 'Sim', p: 1, a: showFeedbackPopup },
      { l: 'Não', a: () => am('Tudo bem, estou à disposição se precisar de mais ajuda!') }
    ]);
  }
  if (keywords.greeting.test(t)) {
    am(`${previousContext}${greetingMsgs[Math.floor(Math.random() * greetingMsgs.length)]}${isPositive ? ' 😊' : ''}`);
    menuPT();
    return;
  }
  ...
  if (keywords.ajuda.test(t)) {
    am('Aqui vai uma lista de tópicos que posso ajudar: segurança, cadastro, Frame premium, suporte humano, denúncias, Termos, dados pessoais. Digite um deles para mais infos!');
    return;
  }
  ...
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
    am(`${previousContext}${notUnderstandMsgs[Math.floor(Math.random() * notUnderstandMsgs.length)]}${isPositive ? ' 😊' : ''}`, [
...
function toggleClosePopup() {
  const closeOverlay = document.getElementById('closeOverlay');
  closeOverlay.style.display = closeOverlay.style.display === 'block' ? 'none' : 'block';
}
...
function searchHistory() {
  ...
  if (!search) return;
  // Small spinner
  results.innerHTML = '<div class="loading-spinner" style="margin: auto; width:20px; height:20px; border-width:2px;"></div>';
  setTimeout(() => {
    results.innerHTML = '';
    messageHistory.filter(msg => msg.text.toLowerCase().includes(search)).forEach(msg => {
      const div = document.createElement('div');
      div.innerHTML = `<span style="font-size:12px; color:var(--typing-color)">${new Date(msg.timestamp).toLocaleString()}</span><br>${sanitize(msg.text)}`;
