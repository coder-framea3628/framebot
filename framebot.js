...
function startChatFlow() {
  ...
  if (userInfo) {
    ...
    const g = getGreeting();
    typingShownThisFlow = false;
    am(`${g}! Fico feliz que esteja aqui. Sou a assistente virtual da Frame Agency e vou auxiliar você nesse atendimento.`, null, 0);
    setTimeout(() => {
      typingShownThisFlow = false;
...
function editProfile() {
  ...
  if (lastEdit) {
    ...
    if (diff < 24 * 60 * 60 * 1000) {
      ...
      showWarningPopup(`Seu perfil já foi editado hoje. Por segurança mudanças estarão sisponíveis novamente em ${remaining}h.`);
      return;
    }
  }
  b.innerHTML = '';
