...
<div class="o" id="ov" role="dialog" aria-modal="true">
  <div class="oc" id="menuOverlay">
    ...
    <input type="text" id="historySearch" placeholder="Buscar no histórico..." oninput="filterHistory()">
...
<div class="o" id="blockOverlay" role="dialog" aria-modal="true">
  <div class="oc">
    ...
    <p>Para suporte, envie email para <a href="mailto:contato@frameag.com"/a></p>
...
const notUnderstoodVariations = [
  ...
  'Hmm, isso não me soa familiar. Tente descrever de outra forma!'
];
const inactivityVariations = [
  'Ainda aí {nome}? Gostaria de saber se ainda posso te ajudar?',
  'Ei, tudo bem por aí? Precisa de mais ajuda?',
  'Parece que você sumiu! Ainda precisa de assistência?'
];
...
function processUserMessage(text) {
  ...
  if (isSensitive) {
    ...
    am('Para privacidade, enviei por email apenas para o time da Frame: contato@frameag.com');
  }
  loadingSpinner.style.display = 'block';
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
  ...
  if (keywords.humano) {
    fh();
    am(`${previousContext}Se preferir, você também pode enviar um email para: <a href="mailto:contato@frameag.com". Nossa equipe está pronta para ajudar com qualquer dúvida mais complexa.${isPositive ? ' 😊' : ''}`);
    sending = false;
    return;
  }
  ...
  if (keywords.cancelamento) {
    am(`${previousContext}Para cancelamentos de renovação automática, revise os termos e solicite enviando um email para: <a href="mailto:contato@frameag.com" ${isPositive ? ' 😊' : ''}`);
    perguntarSatisfacao();
    sending = false;
    return;
  }
...
function perguntarSatisfacao() {
  ...
  am(`Minha explicação ajudou a esclarecer sua dúvida, ${nome}? Estou aqui para explicar mais se necessário.`, [
    ...
    { l: "Não, preciso de mais detalhes", a: () => {
        fh();
        am('Se preferir, envie um email para: <a href="mailto:contato@frameag.com" com mais informações sobre sua dúvida.');
      }}
  ], 600);
}
...
function cmPT() {
  ...
  am("Selecione uma opção abaixo para prosseguirmos:", [
    ...
    { l: "Falar com humano", a: () => {
        fh();
        setTimeout(() => {
          am('Se preferir, você também pode enviar um email para: <a href="mailto:contato@frameag.com".');
        }, 500);
      }},
    { l: "Voltar", a: inicio },
...
function erroCodigo() {
  ...
  am("Problema persiste? Vídeo tutorial: <a href='https://frameag.com/ajuda/cadastro' Assista aqui</a>", [{ l: "Voltar", a: cadastroPT }], 800);
}
function falhaAuthentic() {
  ...
  am("Falar com suporte: <a href='https://t.me/suporteframebot' Telegram 24h</a>", [{ l: "Voltar", a: cadastroPT }], 800);
}
...
function problemaLogin() {
  showTypingIndicator(600);
  am("Para iniciar ou recuperar seu login, acesse a Área de Cadastro em <a href='https://frameag.com/cadastro' e siga os passos intuitivos.", null, 0);
  am("Se ainda tiver dúvidas, assista ao nosso vídeo tutorial completo: <a href='https://frameag.com/ajuda/cadastro'. Ele cobre tudo desde o início.", null, 800);
...
function visitarCatalogo() {
  am("Antes de contratar, explore todas as modelos em nosso catálogo completo: <a href='https://frameag.com/models'. Com perfis verificados e opções detalhadas.", null, 0);
...
function crPT() {
  ...
  am("Selecione uma opção abaixo para prosseguirmos:", [
    ...
    { l: "Termos e Condições", a: () => {
        am("Nossos Termos e Condições detalham como agir e atuar dentro das regras da Frame para evitar qualquer problema. Você pode acessá-los em <a href=\"https://frameag.com/termos\");
      }},
...
function finalizeChat() {
  am('Deseja finalizar esse chat de atendimento? Sem problemas! Fico a disposição para te ajudar sempre que precisar.', [
    ...
    { l: 'Não', a: () => am('Ótimo! Continue perguntando e estou aqui para ajudar.') }
  ]);
}
function filterHistory() {
  ...
  results.innerHTML = '';
  messageHistory.filter(msg => msg.text.toLowerCase().includes(search)).forEach(msg => {
    const item = document.createElement('div');
    item.className = 'history-item';
    item.textContent = msg.text.slice(0, 50) + '...';
...
function selectRating(rating) {
  ...
  if (rating <= 3) {
    ...
    options = ['Agilidade', 'Precisão', 'Amigabilidade', 'Outros'];
  }
...
function submitFeedback() {
  ...
  if (text.length < 20 || text.length > 700) {
    showWarningPopup('Seu feedback deve ter entre 20 e 700 caracteres.');
    return;
  }
