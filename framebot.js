  ...
  screen.innerHTML = `
    ...
    <div class="faq-entry" onclick="openFaqSheet()" role="button" tabindex="0" aria-label="Abrir perguntas frequentes" style="cursor:pointer;">
      ...
      <div class="faq-entry-text">
        ...
        <span>Tire todas as suas dúvidas sobre a Frame Agency</span>
      </div>
...
function processUserMessage(text) {
  ...
  if (isProhibited) {
    ...
    if (badWordCount === 1) {
      ...
      typingShownThisFlow = false;
      am('Sua conversa foi encerrada por violar os Termos de Uso. O time de segurança foi notificado e um bloqueio aplicado.');
...
function cmPT() {
  ...
  am('Como você é contratante, posso ajudar com os seguintes assuntos:', [
    ...
    { l: 'Reportar modelo', a: () => {
      typingShownThisFlow = false;
      am('Levamos a segurança da comunidade muito a sério. Se viu algo suspeito, acesse <a href="https://frameag.com/report"</a> para denunciar anonimamente. Nosso time analisa todas as denúncias com prioridade.');
    }},
...
function falhaAuthentic() {
  ...
  am('Ainda com problemas? Fale com nosso time de atendimento 24h: <a href="https://t.me/suporteframebot"</a>', [{ l: 'Voltar', a: cadastroPT }], 600);
}/
...
function problemaEmail() {
  typingShownThisFlow = false;
  am('Sevocê perdeu acesso ao e-mail: primeiro tente recuperar no provedor (Gmail, Icloud). Se não for possível, podemos ajudar a atualizar o e-mail na sua conta, para isto, pode ser necessário realizar etapas de verificação de segurança.', null, 0);
  am('Para prosseguir:', [
    { l: 'Ir para atendimento', p: 1, a: fh },
    { l: 'Voltar', a: cadastroPT }
  ], 700);
...
function problemaLogin() {
  typingShownThisFlow = false;
  am('Para recuperar seu acesso, vá para <a href="https://frameag.com/cadastro"</a> e siga os passos clicando em "Esqueci minha senha de acesso". Confira também nosso tutorial completo de cadastro disponível aqui: <a href="https://frameag.com/ajuda/cadastro"</a>.', null, 0);
...
function ctPT() {
  ...
  am('Para sua segurança e em conformidade com as diretrizes, preciso confirmar a informação acima antes de prosseguir.');
  setTimeout(() => {
    showAgeCheckWidget(() => {
      typingShownThisFlow = false;
      am('Tudo certo! Como prefere prosseguir?', [
...
function agendarExperiencia() {
  typingShownThisFlow = false;
  am('Para agendar basta selecionar uma modelo no catálogo, iniciar um chat, negociar os detalhes e assinar o contrato enviado por e-mail. A Frame Agency fornece a tecnologia, a negociação é 100% direta e segura entre você e a modelo.', null, 0);
  am('Pronto para começar e conhecer mais de 15mil modelos verificadas?', [
    { l: 'Ir para o catálogo agora ', p: 1, a: () => showRedirectLoading('https://frameag.com/models', true) },
    { l: 'Voltar', a: ctPT }
  ], 600);
}
...
function visitarCatalogo() {
  typingShownThisFlow = false;
  am('Explore perfis de modelos verificadas em <a href="https://frameag.com/models"</a>. Por lá você encontra opções de filtro por idade, altura, cor do cabelo e muito mais.', null, 0);
  am('Decidiu quem deseja contratar? Nosso time pode ajudar:', [{ l: 'Ir para atendimento', p: 1, a: () => showRedirectLoading('https://t.me/suporteframebot?start=catalogo-site', true) }], 500);
}

function verificacaoContratante() {
  typingShownThisFlow = false;
  am('O Frame Authentic verifica identidade com documentos da modelo, selfie + biometria facial ao vivo. Resultado: perfis 100% reais, sem fraudes dentro da plataforma.', null, 0);
  am('Contratantes também podem ser verificados em caso de contestações ou denúncias.', [{ l: 'Entendido', a: ctPT }], 600);
}
