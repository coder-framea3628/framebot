...
const notUnderstoodVariations = [
  'Não consegui identificar sua solicitação. Pode descrever com mais detalhes?',
  'Hmm, não captei bem. Tente novamente ou escolha uma opção abaixo.',
  'Não entendi dessa vez. Me diga mais sobre o que precisa.',
...
const inactivityVariations = [
  ...
  'Só confirmando que estou por aqui{nome}. Tem mais alguma dúvida?'
];
...
function checkFlood() {
  ...
  if (now - lastSendTime < 1000) {
    floodCount++;
    if (floodCount >= 5) {
      showWarningPopup('Você está enviando mensagens muito rápido. Aguarde um momento.');
      return true;
    }
  } else {
    floodCount = 0;
...
function showInitialScreen() {
  ...
  const subtitle = userName
    ...
    : `Bem-vindo ao atendimento virtual da Frame Agency.`;
...
function showEmailWidget(onConfirm) {
  ...
  wrap.id = 'emailWidget';
  wrap.innerHTML = `
    <p style="font-size:13px;font-family:'Inter',sans-serif;color:var(--typing-color);margin:0 0 12px;">Digite seu e-mail para continuar</p>
    <div class="chat-email-input-wrap" id="emailWrap">
      <div class="chat-email-label">Email</div>
      <input type="email" class="chat-email-input" id="chatEmailInput" placeholder="email@exemplo.com" autocomplete="email" aria-label="Seu e-mail" inputmode="email">
    </div>
...
function processUserMessage(text) {
  ...
  if (phoneRegex.test(text) || cpfRegex.test(text)) {
    typingShownThisFlow = false;
    am('Por privacidade, não compartilhe dados pessoais como CPF ou telefone aqui. Posso ajudar com outras questões!');
    sending = false;
    return;
  }

  if (text.length > MAX_CHAR_LIMIT) {
    showWarningPopup(`Sua mensagem é muito longa. Limite de ${MAX_CHAR_LIMIT} caracteres.`);
    sending = false;
    return;
  }
  ...
  if (waitingForName) {
    ...
    if (name.length < 2 || /[0-9]/.test(name)) {
      typingShownThisFlow = false;
      am('Preciso de um nome válido para continuar. Como posso te chamar?');
      waitingForName = true;
      sending = false;
      return;
    }
    ...
    typingShownThisFlow = false;
    am(`Prazer, ${firstName}! Para guardar nosso histórico e continuar te ajudando, qual é o seu e-mail?`);
  ...
  if (keywords.finalizar) {
    ...
    am('Tudo esclarecido? Podemos finalizar o chamado quando você quiser.', [
      ...
      { l: 'Continuar', a: () => {
        typingShownThisFlow = false;
        am('Ótimo! Continue com suas dúvidas e vou te ajudar.');
      }}
    ]);
    sending = false;
    return;
  ...
  if (keywords.seguranca) {
    typingShownThisFlow = false;
    am(`A Frame utiliza autenticação em múltiplas etapas e um time de segurança disponível 24h. Mais detalhes em <a href="https://frameag.com/verificacao" target="_blank" rel="noopener noreferrer" class="link">frameag.com/verificacao</a>.`);
    perguntarSatisfacao();
    sending = false;
    return;
  }
  ...
  if (keywords.contratante) {
    ...
    localStorage.setItem('currentTopic', currentTopic);
    typingShownThisFlow = false;
    am(`Como contratante, você tem acesso a perfis verificados, agendamentos e comunicação segura. O Premium desbloqueia o catálogo completo sem anúncios, lounges VIP e muito mais. Veja em <a href="https://frameag.com/premium" target="_blank" rel="noopener noreferrer" class="link">frameag.com/premium</a>.`);
  ...
  if (keywords.cancelamento) {
    typingShownThisFlow = false;
    am(`Para cancelamentos ou reembolsos, envie um email para <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> com sua solicitação.`);
    perguntarSatisfacao(); sending = false; return;
  }
  ...
  if (keywords.ajuda) {
    typingShownThisFlow = false;
    am('Posso ajudar com: segurança, cadastro, Frame Premium, atendimento humano, criadora, contratante, denúncias, Termos e dados pessoais. O que precisa?', [
...
function menuPT() {
  typingShownThisFlow = false;
  am('Ótimo! Qual é a sua relação com a Frame hoje? Isso me ajuda a direcionar melhor.', [
...
function cmPT() {
  typingShownThisFlow = false;
  am('Como contratante, posso ajudar com:', [
    { l: 'Quero contratar', p: 1, a: ctPT },
    { l: 'Segurança', a: () => {
      typingShownThisFlow = false;
      am('A Frame garante autenticação em múltiplas etapas e monitoramento 24h. Mais em <a href="https://frameag.com/verificacao" target="_blank" rel="noopener noreferrer" class="link">frameag.com/verificacao</a>.');
      setTimeout(perguntarSatisfacao, 600);
    }},
    { l: 'Reportar modelo', a: () => {
      typingShownThisFlow = false;
      am('Acesse <a href="https://frameag.com/report" target="_blank" rel="noopener noreferrer" class="link">frameag.com/report</a> para denunciar anonimamente. Nosso time analisa com prioridade.');
    }},
...
function cadastroPT() {
  ...
  am('Qual situação se aplica ao seu caso?', [
    { l: 'Código de ativação', a: problemaCodigo },
...
function erroCodigo() {
  typingShownThisFlow = false;
  am('Para código inválido: 1) Verifique se copiou corretamente do e-mail. 2) Tente reenviar na página de cadastro. 3) Use outro dispositivo se necessário.');
...
function falhaAuthentic() {
  typingShownThisFlow = false;
  am('Para o Frame Authentic: boa iluminação, rosto nítido, sem óculos ou acessórios. Verifique sua conexão e tente após o prazo indicado.');
...
function problemaCodigo() {
  typingShownThisFlow = false;
  am('Se o código não chegou: verifique a caixa de spam, confirme o e-mail cadastrado e aguarde alguns minutos. Tente reenviar na página de cadastro.', null, 0);
  am('Se persistir, nosso time pode ajudar:', [
...
function problemaEmail() {
  typingShownThisFlow = false;
  am('Se perdeu acesso ao e-mail: primeiro tente recuperar no provedor (Gmail, Outlook). Se não for possível, podemos ajudar a atualizar o e-mail na sua conta Frame (necessário verificação de segurança).', null, 0);
  am('Para prosseguir:', [
    { l: 'Ir para atendimento', p: 1, a: fh },
    { l: 'Voltar', a: cadastroPT }
  ], 700);
...
function problemaLogin() {
  typingShownThisFlow = false;
  am('Para recuperar seu acesso, vá para <a href="https://frameag.com/cadastro" target="_blank" rel="noopener noreferrer" class="link">frameag.com/cadastro</a> e siga os passos. Confira também nosso <a href="https://frameag.com/ajuda/cadastro" target="_blank" rel="noopener noreferrer" class="link">tutorial completo</a>.', null, 0);
  am('Mais dúvidas?', [{ l: 'Voltar', a: cadastroPT }], 600);
}

function premiumPT() {
  typingShownThisFlow = false;
  am('Com o Frame Premium você tem: catálogo completo sem anúncios, acesso a lounges VIP e hotéis parceiros, FramePay e suporte prioritário 24h.', null, 0);
...
function ctPT() {
  ...
  am('Para sua segurança, preciso confirmar uma informação antes de prosseguir.');
  setTimeout(() => {
    showAgeCheckWidget(() => {
      typingShownThisFlow = false;
...
function crPT() {
  typingShownThisFlow = false;
  am('Como criadora, posso ajudar com:', [
...
function submitFeedback() {
  ...
  setTimeout(() => {
    closeFeedbackSheet();
    typingShownThisFlow = false;
    am('Agradecemos pelo feedback! Ele foi recebido e será analisado pelo nosso time.');
...
function finalizeChat() {
  ...
  am('Tudo esclarecido? Podemos finalizar o chamado quando você quiser.', [
    ...
    { l: 'Continuar', a: () => {
      typingShownThisFlow = false;
      am('Ótimo! Continue e vou estar aqui para ajudar.');
    }}
  ]);
}
...
function blockChatForBehavior() {
  ...
  localStorage.removeItem('chatHistory');
  localStorage.removeItem('lastMessage');
  showBlockFixed(`Seu acesso foi bloqueado por ${prevBlocks === 0 ? '1 hora' : '24 horas'} por violar os Termos de Uso.`);
  startBlockTimer();
}
...
function blockChatForUnderage() {
  ...
  inputArea.style.pointerEvents = 'none';
  inputArea.style.opacity = '0.5';
  showBlockFixed('Chat bloqueado por declaração de menoridade. Confirme sua idade para continuar.', true);
}
...
function checkBlockStatus() {
  ...
  if (blockType === 'behavior') {
    if (Date.now() >= blockEndTime) {
      ...
      showBlockFixed('Você ainda está bloqueado por comportamento inadequado.');
      startBlockTimer();
    }
...
function exportHistory() {
  ...
  const footer = [
    '',
    '================================================',
    '  Frame Agency — frameag.com',
    '  Atendimento: contato@frameag.com',
    '================================================'
  ].join('\n');
...
function setupOfflineHandling() {
  ...
  window.addEventListener('offline', () => {
    w.textContent = 'Você está offline';
