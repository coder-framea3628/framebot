...
.edit-back-btn:hover { color: var(--accent-color); background: rgba(171,134,91,0.08); }
.edit-back-btn svg { width: 16px; height: 16px; stroke: currentColor; fill: none; stroke-width: 2; }
/* ===== Danger zone card (delete profile) ===== */
.danger-zone-card {
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.danger-zone-left svg { color: #dc2626; flex-shrink: 0; }
.danger-zone-label {
  font-size: 13px;
  font-weight: 600;
  font-family: 'Inter', sans-serif;
...
.danger-delete-btn:hover { background: #dc2626; color: #fff; }
/* ===== Leave page modal ===== */
.leave-modal {
  position: fixed;
  top: 0; left: 0;
...
.leave-modal-close:hover { background: rgba(171,134,91,0.1); color: var(--accent-color); }
.leave-modal-title {
  font-size: 16px;
  font-weight: 700;
  font-family: 'Montserrat', sans-serif;
...
.leave-modal-exit:hover { background: var(--accent-dark); }
.leave-modal-stay {
  width: 100%;
  padding: 12px;
  background: transparent;
...
.leave-modal-stay:hover { border-color: var(--accent-color); color: var(--accent-color); }
/* ===== Emoji overlay ===== */
.emoji-overlay-box {
  position: absolute;
...
<div class="o" id="menuDialog" role="dialog" aria-modal="true" aria-label="Menu">
  <div class="menu-content" id="menuOverlay">
    ...
    <button class="menu-item" id="soundToggleBtn" onclick="toggleSound()">
...
<div class="sheet-overlay" id="handoffSheet" role="dialog" aria-modal="true" aria-label="Escolher canal de atendimento">
  <div class="sheet-container">
    ...
    <div class="handoff-options">
      <a class="handoff-option" id="handoffTelegramLink" href="https://t.me/suporteframebot?start=chatbot-site-geral" target="_blank" rel="noopener noreferrer" onclick="closeHandoffSheet()">
...
<div class="sheet-overlay" id="faqSheet" role="dialog" aria-modal="true" aria-label="Perguntas Frequentes">
  <div class="sheet-container" style="max-height:88vh; overflow-y:auto;">
    ...
    <div class="sheet-subtitle">Encontre respostas rápidas sobre a Frame Agency</div>
...
<!-- Leave page modal -->
<div class="leave-modal" id="leavePageModal">
  <div class="leave-modal-card">
    <button class="leave-modal-close" onclick="hideLeavePage()">✕</button>
    <p class="leave-modal-title">Tem certeza que deseja sair?</p>
    <p class="leave-modal-sub">Ao fechar a aba, seu atendimento conosco será encerrado.</p>
    <button class="leave-modal-exit" id="leaveConfirmBtn">Sair e finalizar</button>
    <button class="leave-modal-stay" onclick="hideLeavePage()">Continuar aqui</button>
  </div>
</div>
`;
...
// [REQ 4] Som de notificação
let soundEnabled = localStorage.getItem('soundEnabled') !== 'false';

// [REQ 7] Estado anterior ao editProfile
let preEditScreenType = null; // 'initial' | 'chat'

// [REQ 3] Contagem de visitas para variar mensagem de retorno
let visitCount = parseInt(localStorage.getItem('visitCount') || '0');

// Mapeamento de tópicos para labels legíveis [REQ 8]
const topicLabels = {
  'criadora': 'ser criadora na Frame',
  'contratante': 'contratação de modelos',
  'cadastro': 'cadastro e acesso',
  'pagamento': 'pagamentos',
  'premium': 'Frame Premium',
  'seguranca': 'segurança da plataforma',
  'termos': 'Termos e Condições',
  'cancelamento': 'cancelamento e reembolso',
  'dados': 'dados pessoais',
  'suporte': 'suporte técnico',
  'catalogo': 'catálogo de modelos',
  'geral': 'seu último assunto'
};
...
const prohibitedWords = [
  'cu','pau','ppk','goza','chupa','puta','penis','kids','kid','baby','bebe','cp','porno',
...
// ===== Tabela de sinônimos (idioma removido — REQ 10) =====
const synonymTable = {
  greeting: ['oi','ola','olá','ei','hey','salve','e aí','eai','eaí','e ai','bom dia','boa tarde','boa noite','oii','opa','oie','oiii','ola!','oi!','alô','alo'],
  denunciar: ['denunciar','reportar','denuncia','fake','golpe','fraude','suspeito','suspeita','irregular'],
  humano: ['humano','analista','suporte','agente','atendente','pessoa real','falar com alguém','falar com um humano','falar com humano','falar com gente','pessoa humana','operador','assistente humano'],
  ...
  definicoes: ['o que é a frame','o que e a frame','frame agency','quem é','quem e','o que é a plataforma','sobre a frame','about frame','frame agencia'],
  ...
  finalizar: ['finalizar','encerrar','encerrar atendimento','Finalizar atendimento','encerrar chat','fechar','quero sair','tchau','bye','até logo','ate logo'],
...
// [REQ 6] Variações de "não entendi" — mais empáticas e naturais
const notUnderstoodVariations = [
  'Não consegui entender dessa vez, desculpa! Pode tentar de outro jeito ou escolher uma opção?',
  'Hmm, não captei bem o que você precisa. Me conta com mais detalhes?',
  'Essa não ficou clara pra mim. Tenta reformular ou usa uma das opções abaixo?',
  'Poxa, não consegui identificar o que precisa. O que está acontecendo?',
  'Não consegui processar essa. Pode descrever com mais detalhes o que precisa?',
  'Deixa eu entender melhor — pode reformular? Quero ajudar da forma certa.'
];

const inactivityVariations = [
  'Ainda está por aqui{nome}? Posso ajudar com mais alguma coisa?',
  'Oi{nome}, continua por aí? Estou disponível quando precisar.',
  'Ei{nome}, sumiu! 😄 Tem mais alguma dúvida? Fico à disposição.',
  'Tudo certo{nome}? Se precisar de mais alguma coisa, é só chamar.'
];

// [REQ 6] Variações de pergunta de satisfação
const satisfacaoVariations = [
  (nome) => `Isso respondia o que você precisava${nome ? ', ' + nome : ''}?`,
  (nome) => `Ficou claro${nome ? ' pra você, ' + nome : ''}?`,
  (nome) => `Consegui ajudar${nome ? ', ' + nome : ''}?`,
  (nome) => `Tem mais alguma dúvida sobre isso${nome ? ', ' + nome : ''}?`,
  (nome) => `Isso esclareceu a sua dúvida${nome ? ', ' + nome : ''}?`,
  (nome) => `Resolveu o que precisava${nome ? ', ' + nome : ''}?`
];

// [REQ 6] Variações de fechamento positivo
const fechamentoVariations = [
  'Ótimo! Qualquer coisa é só chamar por aqui 😊',
  'Que bom! Fico à disposição se surgir mais alguma dúvida.',
  'Perfeito! Até a próxima 👋',
  'Maravilha! Pode contar com a gente sempre que precisar.',
  'Feliz em ter ajudado! Qualquer dúvida futura, estou aqui.',
  'Show! Se precisar de mais alguma coisa, é só voltar por aqui.'
];

// ===== Utilitários =====
// [REQ 12] Sanitize corrigido — não processa URLs já dentro de atributos href/src
function sanitize(text) {
  text = text.replace(/(https?:\/\/[^\s<"'>]+)/g, (url, offset, str) => {
    // Verificar se a URL está dentro de um atributo HTML (href, src, action)
    const before = str.slice(Math.max(0, offset - 40), offset);
    if (/(?:href|src|action)\s*=\s*["']?$/.test(before)) return url;
    return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="link">${url}</a>`;
  });
  // Converter e-mails em mailto links (não duplicar os já linkados)
...
// [REQ 4] Som de notificação via Web Audio API
...
function toggleSound() {
  ...
  if (btn) {
    const svgOff = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>`;
    const svgOn = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>`;
    btn.innerHTML = soundEnabled
      ? svgOn + ' Som de notificação: ligado'
      : svgOff + ' Som de notificação: desligado';
  }
  showToast(soundEnabled ? 'Som ativado 🔔' : 'Som desativado 🔕', 2000);
}
...
function am(text, btn = null, delay = 0, user = false, timestamp = new Date().toISOString()) {
  ...
  setTimeout(() => {
    ...
    if (user) {
      ...
      m.appendChild(img);
      m.appendChild(bb);
      // [REQ 4] Som para mensagens do bot
      if (isChatInitiated) playNotificationSound();
    }
...
function initChat() {
  ...
  // Atualizar botão de som conforme estado salvo
  if (!soundEnabled) {
    const sBtn = document.getElementById('soundToggleBtn');
    if (sBtn) {
      sBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:20px;height:20px;flex-shrink:0;"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg> Som de notificação: desligado`;
    }
  }
  ...
  // [REQ 2] Input listener com contador de caracteres
  userInput.addEventListener('input', (e) => {
    ...
    // Contador de caracteres (aparece acima de 89% do limite)
    const len = e.target.value.length;
    const counter = document.getElementById('charCounter');
    const threshold = Math.floor(MAX_CHAR_LIMIT * 0.89); // ~222 chars
    if (counter) {
      if (len >= threshold) {
        counter.style.display = 'block';
        counter.textContent = `${len}/${MAX_CHAR_LIMIT}`;
        counter.className = 'char-counter' + (len >= MAX_CHAR_LIMIT ? ' limit' : len >= MAX_CHAR_LIMIT * 0.96 ? ' warn' : '');
      } else {
  ...
  // [REQ 9] Confirmar saída da página quando chat ativo
  window.addEventListener('beforeunload', (e) => {
    if (isChatInitiated && messageHistory.length > 0) {
      e.preventDefault();
      e.returnValue = 'Ao sair, seu atendimento será encerrado.';
      return e.returnValue;
    }
  });

  // Interceptar navegação via popstate (botão voltar do browser)
  ...
  // Adicionar estado no histórico para poder interceptar
  history.pushState(null, '', location.href);
...
function showLeavePageModal() {
  const modal = document.getElementById('leavePageModal');
  if (!modal) return;
  modal.style.display = 'flex';
  modal.classList.add('show');
  document.getElementById('leaveConfirmBtn').onclick = () => {
    hideLeavePage();
    doFinalReset();
  };
}

function hideLeavePage() {
  const modal = document.getElementById('leavePageModal');
  if (!modal) return;
...
function showInitialScreen() {
  ...
  const subtitle = userName
    ? `Que bom ter você de volta. Como posso ajudar?`
  ...
  screen.innerHTML = `
    ...
    <div class="faq-entry" onclick="openFaqSheet()" role="button" tabindex="0" aria-label="Abrir perguntas frequentes" style="cursor:pointer;">
      ...
      <div class="faq-entry-text">
        ...
        <span>Tire todas as suas dúvidas sobre a Frame Agency aqui</span>
      </div>
...
function startChatFlow() {
  ...
  if (userInfo) {
    ...
    // [REQ 3] Mensagem de retorno inteligente com tópico anterior
    const topicKey = currentTopic || localStorage.getItem('currentTopic');
    const showTopicChip = topicKey && topicLabels[topicKey] && visitCount % 3 !== 0;

    typingShownThisFlow = false;

    if (showTopicChip) {
      const topicLabel = topicLabels[topicKey];
      am(`${g}, ${nome}! Da última vez você perguntou sobre ${topicLabel}. Posso continuar te ajudando com isso ou prefere falar sobre outro assunto?`, [
        { l: `Continuar sobre ${topicLabel.split(' ').slice(0,3).join(' ')}`, p: 1, a: () => {
          typingShownThisFlow = false;
      ...
      const genericReturns = [
        `${g}, ${nome}! Que bom ver você de volta. Como posso ajudar hoje?`,
        `${g}, ${nome}! Bem-vindo de volta. O que você precisa hoje?`,
        `${g}, ${nome}! Aqui estou! Como posso ajudar?`
      ];
      const msg = genericReturns[Math.floor(Math.random() * genericReturns.length)];
      am(msg, null, 0);
      setTimeout(() => {
        typingShownThisFlow = false;
        menuPT();
    ...
    const g = getGreeting();
    typingShownThisFlow = false;
    am(`${g}! Como vai? Vou te ajudar a encontrar tudo o que precisa aqui na Frame Agency.`, null, 0);
    setTimeout(() => {
      typingShownThisFlow = false;
...
// [REQ 3] Retomar tópico anterior
function resumeTopic(topicKey) {
  currentTopic = topicKey;
  currentFlowType = topicKey;
  localStorage.setItem('currentTopic', topicKey);

  switch(topicKey) {
    case 'criadora': crPT(); break;
    case 'contratante': cmPT(); break;
    case 'cadastro': cadastroPT(); break;
    case 'premium': premiumPT(); break;
    case 'pagamento':
      ...
      break;
    case 'seguranca':
      am('A Frame Agency utiliza autenticação em múltiplas etapas e possui um time de segurança disponível 24h. Mais detalhes em <a href="https://frameag.com/verificacao" target="_blank" rel="noopener noreferrer" class="link">frameag.com/verificacao</a>.');
      setTimeout(perguntarSatisfacao, 600);
      break;
    case 'termos':
      am('Nossos Termos e Condições estão em <a href="https://frameag.com/termos" target="_blank" rel="noopener noreferrer" class="link">frameag.com/termos</a>. Atuamos em conformidade com a GDPR, Eca Digital e a LGPD.');
      setTimeout(perguntarSatisfacao, 600);
      break;
    default: menuPT(); break;
  }
}
...
function showEmailWidget(onConfirm) {
  ...
  wrap.id = 'emailWidget';
  wrap.innerHTML = `
    <p style="font-size:13px;font-family:'Inter',sans-serif;color:var(--typing-color);margin:0 0 12px;">Informe seu e-mail para continuarmos</p>
    <div class="chat-email-input-wrap" id="emailWrap">
      <div class="chat-email-label">Email</div>
      <input type="email" class="chat-email-input" id="chatEmailInput" placeholder="seunome@email.com" autocomplete="email" aria-label="Seu e-mail" inputmode="email">
    </div>
...
function confirmEmailWidget() {
  ...
  // [REQ 1] Resetar flag junto com o desaparecimento do widget
...
function processUserMessage(text) {
  ...
  if (isProhibited) {
    ...
    if (badWordCount === 1) {
      ...
      typingShownThisFlow = false;
      am('Precisei ocultar sua mensagem mais uma vez. Se continuar assim, nosso chat será encerrado por segurança.');
    } else {
      typingShownThisFlow = false;
      am('Sua conversa foi encerrada por violar os Termos de Uso da plataforma. O time de segurança foi notificado.');
  ...
  // [REQ 2] Limite de 250 caracteres
  if (text.length > MAX_CHAR_LIMIT) {
    showWarningPopup(`Sua mensagem é muito longa. O limite é de ${MAX_CHAR_LIMIT} caracteres — você usou ${text.length}.`);
    sending = false;
    return;
  }
  ...
  if (waitingForName) {
    ...
    setTimeout(() => showEmailWidget((email) => {
      ...
      // waitingForEmail já resetado em confirmEmailWidget
      setTimeout(() => {
        typingShownThisFlow = false;
        menuPT();
      }, 600);
  ...
  // [REQ 1] Se waitingForEmail ainda estiver true (edge case), orientar e não travar
  if (waitingForEmail) {
    typingShownThisFlow = false;
  ...
  if (keywords.logout) {
    ...
    typingShownThisFlow = false;
    am('Os dados foram removidos. Inicie o atendimento novamente quando quiser.');
  ...
  if (keywords.termos) {
    ...
    localStorage.setItem('currentTopic', currentTopic);
    typingShownThisFlow = false;
    am(`Nossos Termos e Condições estão disponíveis em <a href="https://frameag.com/termos" target="_blank" rel="noopener noreferrer" class="link">frameag.com/termos</a>. Para dúvidas de privacidade, atuamos em conformidade com a GDPR, Eca Digital e a LGPD.`);
    perguntarSatisfacao();
    sending = false;
    return;
  }
  ...
  if (keywords.dados) {
    ...
    localStorage.setItem('currentTopic', currentTopic);
    typingShownThisFlow = false;
    am(`Tratamos seus dados com segurança e transparência. Para acessar, corrigir ou solicitar exclusão, envie e-mail para <a href="mailto:privacidade@frameag.com" class="link">privacidade@frameag.com</a>.`);
    perguntarSatisfacao(); sending = false; return;
  }
  ...
  if (keywords.pagamento) {
    currentTopic = 'pagamento';
    localStorage.setItem('currentTopic', currentTopic);
    typingShownThisFlow = false;
    am(`Para dúvidas sobre pagamentos, acesse sua área de login em <a href="https://frameag.com/login" target="_blank" rel="noopener noreferrer" class="link">frameag.com/login</a> ou entre em contato: <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a>.`);
    perguntarSatisfacao(); sending = false; return;
  }
  if (keywords.cancelamento) {
    currentTopic = 'cancelamento';
    localStorage.setItem('currentTopic', currentTopic);
    typingShownThisFlow = false;
    am(`Para cancelamentos ou reembolsos, envie um e-mail para <a href="mailto:contato@frameag.com" class="link">contato@frameag.com</a> com sua solicitação e o time poderá te ajudar.`);
    perguntarSatisfacao(); sending = false; return;
  }
  if (keywords.suporte) { fh(); sending = false; return; }
  if (keywords.cadastro) { currentTopic = 'cadastro'; localStorage.setItem('currentTopic', currentTopic); cadastroPT(); sending = false; return; }
  if (keywords.premium) { currentTopic = 'premium'; localStorage.setItem('currentTopic', currentTopic); premiumPT(); sending = false; return; }
  if (keywords.ajuda) {
    typingShownThisFlow = false;
  ...
  // [REQ 6] Fallback com mais variações empáticas
  typingShownThisFlow = false;
...
function cmPT() {
  ...
  am('Como você é contratante, posso ajudar com os seguintes assuntos:', [
    ...
    { l: 'Reportar modelo', a: () => {
      typingShownThisFlow = false;
      // [REQ 12] Link corrigido
      am('Levamos a segurança da comunidade muito a sério. Se viu algo suspeito, acesse <a href="https://frameag.com/report" target="_blank" rel="noopener noreferrer" class="link">frameag.com/report</a> para denunciar anonimamente. Nosso time analisa todas as denúncias com prioridade.');
    }},
...
function falhaAuthentic() {
  ...
  am('Ainda com problemas? <a href="https://t.me/suporteframebot" target="_blank" rel="noopener noreferrer" class="link">Suporte Telegram 24h</a>', [{ l: 'Voltar', a: cadastroPT }], 600);
}
...
function problemaEmail() {
  typingShownThisFlow = false;
  am('Se perdeu acesso ao e-mail cadastrado, fique tranquilo! Primeiro tente recuperar no seu provedor (Gmail, iCloud, Outlook). Se não for possível, podemos ajudar a atualizar o e-mail na sua conta. Na maioria dos casos, será necessário realizar uma verificação de segurança.', null, 0);
  am('Para prosseguir:', [
    { l: 'Ir para atendimento', p: 1, a: fh },
    { l: 'Voltar', a: cadastroPT }
  ], 700);
...
function problemaLogin() {
  typingShownThisFlow = false;
  am('Para recuperar seu acesso, vá para <a href="https://frameag.com/cadastro" target="_blank" rel="noopener noreferrer" class="link">Área de cadastro</a> e siga os passos. Confira também nosso <a href="https://frameag.com/ajuda/cadastro" target="_blank" rel="noopener noreferrer" class="link">tutorial completo</a>.', null, 0);
  am('Mais dúvidas?', [{ l: 'Voltar', a: cadastroPT }], 600);
}

function premiumPT() {
  typingShownThisFlow = false;
  currentTopic = 'premium';
  localStorage.setItem('currentTopic', currentTopic);
...
function ctPT() {
  typingShownThisFlow = false;
  currentTopic = 'catalogo';
  localStorage.setItem('currentTopic', currentTopic);
  // Verificação de maioridade via checkbox conversacional
  am('Para sua segurança e conformidade com as diretrizes, preciso confirmar a informação acima antes de prosseguir.');
  setTimeout(() => {
    showAgeCheckWidget(() => {
      typingShownThisFlow = false;
      am('Confirmado! Como prefere prosseguir?', [
        { l: 'Como agendar na plataforma?', a: agendarExperiencia },
        { l: 'Visitar catálogo de modelos', a: visitarCatalogo },
        { l: 'Quero ser Premium', a: premiumPT },
...
function visitarCatalogo() {
  typingShownThisFlow = false;
  am('Explore perfis de modelos verificadas em <a href="https://frameag.com/models" target="_blank" rel="noopener noreferrer" class="link">frameag.com/models</a>. Utilize filtros, escolha opções de atendimento e muito mais.', null, 0);
  am('Decidiu quem contratar? Nosso time pode ajudar:', [{ l: 'Ir para atendimento', p: 1, a: () => showRedirectLoading('https://t.me/suporteframebot?start=catalogo-site', true) }], 500);
}

// [REQ 12] verificacaoContratante corrigida — "Entendido" não reabre o fluxo de idade
function verificacaoContratante() {
  typingShownThisFlow = false;
  am('O Frame Authentic verifica a identidade das acompanhantes com documentos, selfie e biometria facial ao vivo. O resultado? Perfis 100% reais e uma plataforma onde fraude é coisa do passado.', null, 0);
  am('Contratantes também podem ser verificados em caso de contestações.', [
    { l: 'Entendido', a: () => {
      typingShownThisFlow = false;
      am('Perfeito! Como prefere continuar?', [
        { l: 'Como agendar na plataforma?', a: agendarExperiencia },
        { l: 'Visitar catálogo de modelos', a: visitarCatalogo },
        { l: 'Quero ser Premium', a: premiumPT },
        { l: 'Voltar', a: cmPT }
...
function crPT() {
  ...
  am('Como você é uma criadora, posso ajudar com os assuntos abaixo:', [
    { l: 'Como me cadastrar', a: () => {
      typingShownThisFlow = false;
      am('Acesse <a href="https://frameag.com/cadastro" target="_blank" rel="noopener noreferrer" class="link">Área de cadastro</a> e siga as instruções. O processo é simples, seguro e direto aqui no site.');
      setTimeout(perguntarSatisfacao, 600);
    }},
    { l: 'Como funciona a verificação', a: () => {
      typingShownThisFlow = false;
      am('Você precisa ter mais de 18 anos, passar pelo Frame Authentic (verificação de identidade) e aceitar os nossos Termos. Isso garante ainda mais segurança para todos os usuários.');
      setTimeout(perguntarSatisfacao, 600);
    }},
    { l: 'Termos e Condições', a: () => {
      typingShownThisFlow = false;
      am('Nossos Termos estão disponíveis em <a href="https://frameag.com/termos" target="_blank" rel="noopener noreferrer" class="link">frameag.com/termos</a>. Recomendamos a leitura antes de utilizar os serviços da Plataforma.');
    }},
    { l: 'O que é a Frame', a: () => {
      typingShownThisFlow = false;
...
function gerenciarPerfil() {
  typingShownThisFlow = false;
  am('Na sua BrandPage você pode editar suas mídias, bio, medidas e adicionar redes sociais. Para destaque no catálogo, veja os planos de impulsionamento disponíveis na sua dashboard.');
...
function framePayments() {
  typingShownThisFlow = false;
  am('Com o FramePay você recebe diretamente no seu bot do Telegram e saque via PIX, sem taxas escondidas. Com anti-fraude e ainda mais segurança em cada transação.');
...
function fh() {
  typingShownThisFlow = false;
  am('Vou conectar você com um agente do nosso time! Escolha o canal mais fácil pra você aqui em baixo:', [
...
// ===== Perguntar satisfação (variações) — REQ 6 =====
function perguntarSatisfacao() {
  const nome = userInfo ? userInfo.name.split(' ')[0] : '';
  const fn = satisfacaoVariations[Math.floor(Math.random() * satisfacaoVariations.length)];
  typingShownThisFlow = false;
  am(fn(nome), [
    { l: 'Sim, tudo certo!', p: 1, a: () => {
      typingShownThisFlow = false;
      // [REQ 6] Variação no fechamento positivo
      const fechamento = fechamentoVariations[Math.floor(Math.random() * fechamentoVariations.length)];
      am(fechamento);
    }},
    { l: 'Preciso de mais ajuda', a: () => { fh(); }}
  ], 500);
}
...
function clearChatHistory() {
  closeMenu();
  showConfirm('Tem certeza que deseja excluir seu histórico? Ao continuar, ele não poderá ser recuperado.', () => {
    messageHistory = [];
...
// ===== Edit Profile (com botão voltar, deleção e lógica de retorno) — REQ 7 e 11 =====
function editProfile() {
  closeMenu();
  if (!userInfo) {
    showWarningPopup('Poxa, não encontrei nenhum perfil. Inicie um atendimento para criar seu perfil por aqui.');
    return;
  }
  // Verificar limite de edição (24h)
  ...
  if (lastEdit) {
    ...
    if (diff < 24 * 60 * 60 * 1000) {
      ...
      showWarningPopup(`Seu perfil já foi editado hoje. Por segurança, mudanças estarão disponíveis novamente em ${remaining}h.`);
      return;
    }
  }

  // [REQ 7] Salvar estado anterior
  preEditScreenType = (isChatInitiated && messageHistory.length > 0) ? 'chat' : 'initial';

  b.innerHTML = '';
  addChatStarter();
  // [REQ 7] Esconder input area
  inputArea.style.display = 'none';
  ...
  form.innerHTML = `
    <button class="edit-back-btn" onclick="goBackFromEdit()">
    ...
    <!-- [REQ 11] Zona sensível — excluir perfil -->
    <div class="danger-zone-card">
      <div class="danger-zone-left">
        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 7v6m0 3.5v.5"/></svg>
        <span class="danger-zone-label">Zona sensível</span>
      </div>
      <button class="danger-delete-btn" onclick="deleteProfile()">Excluir perfil</button>
    </div>
  `;
  b.appendChild(form);
}

// [REQ 7] Voltar do editProfile para o estado anterior
function goBackFromEdit() {
  b.innerHTML = '';
  if (preEditScreenType === 'chat') {
    inputArea.style.display = 'block';
    loadChat();
  } else {
    inputArea.style.display = 'none';
...
function saveProfile() {
  ...
  // [REQ 7] Retornar ao estado anterior após salvar
  setTimeout(() => {
    goBackFromEdit();
  }, 900);
}

// [REQ 11] Excluir perfil com cooldown de 48h
function deleteProfile() {
  // Verificar cooldown de 48h
  const lastDelete = localStorage.getItem('lastProfileDelete');
  if (lastDelete) {
    const diff = Date.now() - parseInt(lastDelete);
    const cooldown48h = 48 * 60 * 60 * 1000;
    if (diff < cooldown48h) {
      const remainingH = Math.ceil((cooldown48h - diff) / (60 * 60 * 1000));
      showWarningPopup(`Você já solicitou exclusão recentemente. Esta ação só pode ser feita novamente em ${remainingH}h.`);
      return;
    }
  }
  showConfirm('Tem certeza que deseja excluir seu perfil? Todos os seus dados e histórico serão removidos permanentemente.', () => {
    // Limpar tudo
    localStorage.removeItem('userInfo');
    localStorage.removeItem('chatHistory');
    localStorage.removeItem('lastMessage');
    ...
    localStorage.removeItem('lastProfileEdit');
    localStorage.removeItem('visitCount');
    localStorage.removeItem('draft');
    localStorage.setItem('lastProfileDelete', Date.now().toString());
    ...
    waitingForName = false;
    waitingForEmail = false;
    preEditScreenType = null;

    showToast('Perfil excluído com sucesso.', 3000);
    setTimeout(() => {
      inputArea.style.display = 'none';
      showInitialScreen();
    }, 800);
...
// ===== Bottom Sheets =====
// [REQ 8] openHandoffSheet atualiza URL com tópico atual
function openHandoffSheet() {
  closeMenu();
  // Passar contexto do tópico no link do Telegram
  const topic = currentTopic || 'geral';
  const tgLink = document.getElementById('handoffTelegramLink');
  if (tgLink) tgLink.href = `https://t.me/suporteframebot?start=chatbot-site-${topic}`;
...
// [REQ 5] showRedirectLoading com 800ms (era 2000ms)
...
function resetFlow() {
  ...
  currentTopic = null;
  waitingForName = false;
  // [REQ 1] Garantir reset do flag ao resetar
  waitingForEmail = false;
...
function handleKeyboardShortcuts(e) {
  if (e.key === 'Escape') {
    // [REQ 7] Se estiver no modo edição de perfil, voltar
    if (document.querySelector('.profile-edit-form')) {
      goBackFromEdit();
      return;
    }
    // [REQ 1] Se email widget estiver visível, limpar flag e remover widget
    const ew = document.getElementById('emailWidget');
    if (ew) {
      ew.style.animation = 'fadeOut 0.2s ease forwards';
      setTimeout(() => { if (ew.parentNode) ew.remove(); }, 200);
      waitingForEmail = false;
      window._emailConfirmCallback = null;
      return;
    }
...
// ===== Init =====
// [REQ 8 — nota para builder do Telegram] Tópicos disponíveis no parâmetro ?start=chatbot-site-{topic}:
// criadora | contratante | cadastro | pagamento | premium | seguranca | termos | cancelamento | dados | suporte | catalogo | geral
function initChat() {
  try {
    const raw = localStorage.getItem('chatHistory');
    messageHistory = raw ? (JSON.parse(LZString.decompress(raw)) || []) : [];
  ...
  // [REQ 2] Input com contador de caracteres
  userInput.addEventListener('input', (e) => {
    resetInactivityTimer();
    localStorage.setItem('draft', e.target.value);
    ...
    const len = e.target.value.length;
    const counter = document.getElementById('charCounter');
    const threshold = Math.floor(MAX_CHAR_LIMIT * 0.89);
    if (counter) {
      if (len >= threshold) {
        counter.style.display = 'block';
        counter.textContent = `${len}/${MAX_CHAR_LIMIT}`;
  ...
  // [REQ 9] Confirmar saída quando chat ativo
  window.addEventListener('beforeunload', (e) => {
    if (isChatInitiated && messageHistory.length > 0) {
      e.preventDefault();
      e.returnValue = 'Ao sair, seu atendimento será encerrado.';
  ...
  // Interceptar botão voltar do browser quando chat ativo
  window.addEventListener('popstate', () => {
    if (isChatInitiated && messageHistory.length > 0) {
      history.pushState(null, '', location.href);
      showLeavePageModal();
