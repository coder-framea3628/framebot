...
.msg.user-msg{justify-content:flex-end} /* usuário à direita */
...
.oc{background:#fff;padding:24px;border-radius:10px;text-align:center;display:flex;flex-direction:column;gap:14px;min-width:240px;position:relative;top:50px} /* overlay empurrada mais pra baixo */
...
.semibold{font-weight:600} /* para texto customizado */
...
<div class="c" id="cb">
  ...
  <div class="input-box">
    ...
    <button class="send-btn" id="sendBtn">
      <svg viewBox="0 0 24 24"><path d="M2 12l18-10v8h-18v4h18v8z"/></svg>
    </button>
  </div>
  <div class="f">
...
function typingSimulator() {
  ...
  typingLabel.innerHTML = 'Digitando...';
  const typingBubble = document.createElement('div');
  typingBubble.className = 'typing-bubble';
  typingBubble.innerHTML = '<div class="bb"><div class="dots"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div></div>';
  b.appendChild(typingLabel);
  b.appendChild(typingBubble);
  setTimeout(() => {
    typingLabel.remove();
    typingBubble.remove();
  }, 2000); // Duração do simulador de digitação
}
...
document.getElementById('userInput').addEventListener('keypress', e => {
  if (e.key === 'Enter' && e.target.value.trim() !== "") {
    ...
    e.target.value = "";
    am(text, null, 0, true);
    typingSimulator(); // Chama o simulador de digitação
...
function cmPT() {
  ...
  am("Selecione uma opção:", [
    ...
    { l: "Reportar modelo anonimamente", a: () => { 
        ...
        typingSimulator(); // Chama o simulador de digitação
        setTimeout(() => {
          am("Obrigado por reportar. Estamos sempre atentos!", null, 0);
          perguntarSatisfacao();
        }, 2000); // Duração do simulador de digitação
      }},
    ...
    { l: "Voltar", a: inicio },
    { l: "Quero me tornar Premium", p: 1, a: premiumPT } // última opção
  ], 800);
}

// ===== Novo fluxo Premium =====
function premiumPT() {
  am('No <span class="semibold">Premium</span>, você desbloqueia vantagens que ninguém mais pode ter. Deixa eu te contar:', null, 0);
  am('Acesso ao catálogo completo, Galerias Sexy, atendimento prioritário e muito mais.', null, 700);
  am('Para se tornar <span class="semibold">Premium</span>, clique abaixo.', [{ l: "Quero ser Premium", p: 1, a: () => window.open('https://frameag.com/premium','_blank') }], 1400);
}
...
function ctPT() {
  ...
  am("Você possui 18 anos ou mais?", [
    { l: "Sim", p: 1, a: () => { 
        am('Certo. Antes de contratar, você pode conhecer todas as modelos em nosso <a href="https://frameag.com/models" target="_blank" class="link">catálogo completo</a>.', null, 0);
        am("Mas, se quiser contratar, fale com o nosso time de atendimento:", [{ l: "Ir para atendimento 24h", p: 1, a: () => open('https://t.me/suporteframebot?start=catalogo-site', '_blank') }], 500);
        perguntarSatisfacao();
      }},
    { l: "Não", a: () => { 
