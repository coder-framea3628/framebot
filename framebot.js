...
.offline-popup{position:absolute;bottom:100%;left:50%;transform:translateX(-50%);background:#AC865C;color:#fff;padding:8px 12px;border-radius:14px;display:none;align-items:center;gap:6px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.3)}
.offline-popup img{width:18px;height:18px}
...
<div class="c" id="cb">
  ...
  <div class="offline-popup" id="offlinePopup">
    <img src="https://cdn-icons-png.flaticon.com/512/159/159469.png" alt="Offline"> Offline. Clique para reiniciar
  </div>
</div>
<div class="o" id="ov">
...
const b = document.getElementById('bd'),
      ...
      offlinePopup = document.getElementById('offlinePopup');

let idleTimer;
function resetIdleTimer(){
  clearTimeout(idleTimer);
  idleTimer = setTimeout(()=>{offlinePopup.style.display='flex';},15000);
}
document.addEventListener('mousemove', resetIdleTimer);
document.addEventListener('keypress', resetIdleTimer);
resetIdleTimer();

offlinePopup.addEventListener('click', ()=>{
...
function perguntarSatisfacao() {
  ...
  pergunta.id = 'pergSatisfacao';
  am("Minha explicação acima te ajudou?", [
    { l: "Sim, tudo esclarecido", p: 1, a: () => {
        document.getElementById('pergSatisfacao').remove();
        am("Perfeito! Agradecemos pelo contato, estamos sempre à disposição.<br>Ou, se preferir, envie um e-mail para <b>contato@contato.com.br</b>", [
          { l: "Reiniciar", a: inicio }
        ]);
