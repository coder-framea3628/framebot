...
function initChat() {
  ...
  setupEmojiGrid();
  document.querySelectorAll('.o').forEach(overlay => {
  ...
  if (!userInfo) {
    b.innerHTML = `
      <div class="mini-form">
        ...
        <input type="date" id="birthdateInput" placeholder="Data de nascimento (DD/MM/AAAA)" aria-label="Data de nascimento">
...
function saveUserInfo() {
  ...
  if (birthdate) {
    ...
    if (age < 18) {
      showWarningPopup('Você deve ter pelo menos 18 anos para prosseguir.');
      return;
    }
  }
  ...
  setTimeout(() => {
    ...
    if (btn) {
      ...
      btn.forEach(o => {
        ...
        b2.innerText = o.l;
        const lockKey = o.l.toLowerCase().replace(/\s/g, '');
        b2.onclick = () => {
          if (!buttonClickLocks[lockKey]) {
            buttonClickLocks[lockKey] = true;
            o.a();
            setTimeout(() => { buttonClickLocks[lockKey] = false; }, 5000);
          }
        };
    ...
    if (!user && text && !text.startsWith('Olá') && !text.startsWith('Boas-vindas') && !text.startsWith('É ótimo')) {
      ...
      copyIcon.onclick = () => {
        navigator.clipboard.writeText(text).then(() => {
          ...
          copyIcon.appendChild(feedback);
          setTimeout(() => feedback.remove(), 2000);
        });
      };
      bb.appendChild(copyIcon);
      if (!/Mobi|Android/i.test(navigator.userAgent)) {
        const popup = document.createElement('div');
        popup.className = 'copy-popup';
        popup.innerHTML = '<span class="copy-btn">📋 Copiar</span>';
        popup.querySelector('.copy-btn').onclick = copyIcon.onclick;
        bb.appendChild(popup);
      }
    }
    if (user) {
      m.append(bb);
...
function processUserMessage(text) {
  ...
  const friendlyTone = sentiment > 0 ? ' 😊' : '';
  const keywords = {
  ...
  if (keywords.bomdia.test(t)) {
    am(`Bom dia! Tudo bem? Como posso ajudar você hoje?${friendlyTone}`);
    menuPT();
    return;
  }
  if (keywords.boatarde.test(t)) {
    am(`Boa tarde! Tudo bem? Como posso ajudar você hoje?${friendlyTone}`);
    menuPT();
    return;
  }
  if (keywords.boanoite.test(t)) {
    am(`Boa noite! Tudo bem? Como posso ajudar você hoje?${friendlyTone}`);
    menuPT();
    return;
  }
  if (keywords.greeting.test(t)) {
    am(`Olá! Tudo bem? Como posso ajudar você hoje?${friendlyTone}`);
    menuPT();
    return;
  }
  ...
  if (keywords.criadora.test(t)) {
    ...
    am('Criadoras podem ser agenciadas, recebendo suporte completo, ou anunciantes independentes com ferramentas de anúncio. Para se cadastrar, acesse <a href="https://frameag.com/cadastro" target="_blank" class="link">frameag.com/cadastro</a>, confirme ser maior de 18 anos e passe pelo Frame Authentic.');
    perguntarSatisfacao();
    return;
  }
  ...
  if (keywords.pagamento.test(t)) {
    am('Para dúvidas sobre pagamentos, acesse sua área de login ou entre em contato com o time via suporte.');
    perguntarSatisfacao();
    return;
  }
  if (keywords.cancelamento.test(t)) {
    am('Para cancelamentos de renovação automática, revise os termos e solicite via email para: contato@frameag.com.');
    perguntarSatisfacao();
    return;
  }
  ...
  setTimeout(() => {
    loadingSpinner.style.display = 'none';
    am(`Não consegui entender sua solicitação. Vamos tentar novamente? Me descreva mais detalhes sobre o que precisa, como segurança e cadastro.${friendlyTone}`, [
...
function analyzeSentiment(text) {
  const positiveWords = ['bom', 'otimo', 'feliz', 'obrigado', 'legal', 'maravilha'];
...
function problemaCodigo() {
  am("Se o código de ativação não chegou, aqui vão algumas dicas rápidas para verificar:<br><br>1. <strong>Verifique a caixa de Spam</strong> ou lixo eletrônico do seu email.<br>2. <strong>Confirme se o e-mail está correto</strong> no site.<br>3. <strong>Aguarde alguns minutos</strong>, pois pode haver atraso.<br>4. <strong>Tente reenviar o código</strong> diretamente pela página de cadastro.");
...
function problemaEmail() {
  am("Se você perdeu acesso ao e-mail usado no cadastro, não se preocupe, há soluções:<br><br>1. <strong>Tente recuperar a conta de e-mail</strong> diretamente no provedor (como Gmail ou Outlook).<br>2. <strong>Se não for possível</strong>, podemos ajudar a atualizar o e-mail na sua conta Frame.<br><br>Importante: Para segurança, você precisará confirmar dados pessoais e passar pela verificação Frame Authentic antes da atualização.");
  ...
  showConfirm('Tem certeza que deseja editar seu perfil? Alterações serão salvas imediatamente na próxima etapa.', () => {
    b.innerHTML = `
      <div class="mini-form">
        ...
        <input type="text" id="nameInput" value="${userInfo ? userInfo.name : ''}" placeholder="${userInfo ? userInfo.name : 'Seu nome'}" aria-label="Digite seu nome" style="color: var(--typing-color);">
        <input type="email" id="emailInput" value="${userInfo ? userInfo.email : ''}" placeholder="${userInfo ? userInfo.email : 'Seu e-mail'}" aria-label="Digite seu e-mail" style="color: var(--typing-color);">
        <input type="date" id="birthdateInput" value="${userInfo ? userInfo.birthdate : ''}" placeholder="${userInfo && userInfo.birthdate ? userInfo.birthdate : 'Data de nascimento (DD/MM/AAAA)'}" aria-label="Data de nascimento" style="color: var(--typing-color);">
